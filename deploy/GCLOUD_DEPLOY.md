...
#### All code-referenced secrets are now in Secret Manager and wired above
Every secret read by `server/src` exists in Secret Manager and is included in the
`--set-secrets` list above (35 total). The app boots on `DATABASE_URL` + `JWT_SECRET`
alone; the rest enable their respective features (SMTP, Gemini, Maps, Twilio SMS OTP,
Meta WhatsApp send/OTP/webhook, web push, etc.).
