...
export default function Vehicles() {
  ...
  const statusColor = (s: string) => ({ active: '#22c55e', maintenance: '#f7c948', inactive: '#ef4444' }[s] || '#9fb0c7');
  const statusBg = (s: string) => ({ active: 'rgba(34,197,94,.12)', maintenance: 'rgba(247,201,72,.12)', inactive: 'rgba(239,68,68,.12)' }[s] || 'rgba(159,176,199,.12)');
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#9fb0c7', fontSize: 13 }}>{vehicles.filter(v => v.status === 'active').length}/{vehicles.length} vehicles active</p>
        </div>
            ...
            <div key={v.id} className="glass-card" style={{ padding: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#9fb0c7', marginTop: 2 }}>{v.type} · {v.capacity} m³</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button onClick={() => openEdit(v)} style={{ padding: 5, background: 'rgba(56,189,248,.1)', border: '1px solid rgba(56,189,248,.2)', borderRadius: 7, cursor: 'pointer', color: '#38bdf8' }}><Edit2 size={12} /></button>
                    <button onClick={() => remove(v)} style={{ padding: 5, background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 7, cursor: 'pointer', color: '#ef4444' }}><Trash2 size={12} /></button>
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 12 }}>
                <div style={{ color: '#9fb0c7' }}>Driver: <span style={{ color: '#eef5ff', fontWeight: 600 }}>{v.driverName || 'Unassigned'}</span></div>
                {v.driverPhone && <div style={{ color: '#9fb0c7' }}>Phone: <span style={{ color: '#eef5ff' }}>{v.driverPhone}</span></div>}
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: insWarning ? '#f7c948' : '#9fb0c7' }}>
                  {insWarning && <AlertTriangle size={12} color="#f7c948" />}
                  Insurance: <span style={{ color: insWarning ? '#f7c948' : '#eef5ff', fontWeight: insWarning ? 700 : 400 }}>
                    {v.insuranceExpiry ? `${v.insuranceExpiry} (${insExpiry}d)` : '—'}
                  </span>
                </div>
                <div style={{ color: '#9fb0c7' }}>Last Service: <span style={{ color: '#eef5ff' }}>{v.lastService || '—'}</span></div>
              </div>
            </div>
          );
        })}
        {filtered.length === 0 && (
          <div style={{ gridColumn: '1/-1', padding: 40, textAlign: 'center', color: '#9fb0c7' }}>No vehicles found</div>
        )}
      </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#0d1930', border: '1px solid #263449', borderRadius: 16, padding: 28, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
