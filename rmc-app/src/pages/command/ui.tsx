...
export function SocialLinks({ compact }: { compact?: boolean }) {
  // Links come from the admin-editable "Social Appearance" settings served in
  // the public bootstrap config (with defaults baked into the provider).
  const { socialLinks } = useConfig();
  const SOCIALS: { name: string; color: string; href: string; icon: ReactNode }[] = [
    { name: 'Instagram', color: '#E4405F', href: socialLinks.instagram, icon: <Svg d={BRAND_PATHS.instagram} /> },
    { name: 'YouTube', color: '#FF0000', href: socialLinks.youtube, icon: <Svg d={BRAND_PATHS.youtube} /> },
    { name: 'Facebook', color: '#1877F2', href: socialLinks.facebook, icon: <Svg d={BRAND_PATHS.facebook} /> },
    { name: 'WhatsApp', color: '#25D366', href: socialLinks.whatsapp, icon: <Svg d={BRAND_PATHS.whatsapp} /> },
  ].filter(s => s.href);
  return (
