    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: 'var(--muted)', fontSize: 13 }}>{drivers.filter(d => d.isActive).length} active · {drivers.length} total</p>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid var(--line)' }}>
                {['Driver', 'Phone', 'License No', 'License Expiry', 'Status', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '12px 14px', color: 'var(--muted)', fontWeight: 600, textAlign: 'left', fontSize: 12 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(d => {
                ...
                return (
                  <tr key={d.id} style={{ borderBottom: '1px solid rgba(38,52,73,.4)' }}>
                    ...
                    <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontSize: 12, color: 'var(--muted)' }}>{d.licenseNo || '—'}</td>
                    <td style={{ padding: '11px 14px', fontSize: 12 }}>
                      {d.licenseExpiry ? (
                        <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: licWarning ? 'var(--gold)' : 'var(--muted)' }}>
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{
                        ...
                        color: d.isActive ? 'var(--green)' : 'var(--red)',
                        background: d.isActive ? 'rgba(34,197,94,.12)' : 'rgba(239,68,68,.12)',
                      }}>{d.isActive ? 'Active' : 'Inactive'}</span>
                    </td>
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 5 }}>
                        <button onClick={() => openEdit(d)} style={{ padding: '4px 8px', background: 'rgba(56,189,248,.1)', border: '1px solid rgba(56,189,248,.2)', borderRadius: 7, cursor: 'pointer', color: 'var(--blue)' }}><Edit2 size={12} /></button>
                        <button onClick={() => remove(d)} style={{ padding: '4px 8px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 7, cursor: 'pointer', color: 'var(--red)' }}><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                );
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} style={{ padding: '30px', textAlign: 'center', color: 'var(--muted)' }}>No drivers found</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, gridColumn: '1/-1' }}>
                ...
                <label htmlFor="active" style={{ fontSize: 13, color: 'var(--text)', cursor: 'pointer' }}>Active Driver</label>
              </div>
            </div>
