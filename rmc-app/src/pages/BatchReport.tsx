    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#9fb0c7', fontSize: 13 }}>{batches.length} batch records total</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 18 }}>
        {[
          { label: "Today's Batches", value: summary.totalBatches, unit: '', color: '#38bdf8' },
          { label: "Today's Production", value: Number(summary.totalQty || 0).toFixed(1), unit: 'm³', color: '#22c55e' },
          { label: 'Cement Used', value: summary.totalCement, unit: 'bags', color: '#f7c948' },
          ...
          <div key={label} className="glass-card" style={{ padding: '16px 14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11, color: '#9fb0c7', fontWeight: 600 }}>{label}</span>
            </div>
            <div style={{ fontSize: 20, fontWeight: 900, color: '#eef5ff' }}>{value} <span style={{ fontSize: 12, fontWeight: 500, color: '#9fb0c7' }}>{unit}</span></div>
          </div>
        ))}
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {Object.entries(gradeBreakdown).sort((a, b) => b[1] - a[1]).map(([grade, qty]) => (
              <div key={grade} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ width: 40, fontSize: 12, fontWeight: 700, color: '#38bdf8' }}>{grade}</span>
                <div style={{ flex: 1, background: 'rgba(255,255,255,.05)', borderRadius: 999, overflow: 'hidden', height: 8 }}>
                  <div style={{ width: `${(qty / totalQtyAll) * 100}%`, height: '100%', background: 'linear-gradient(90deg,#38bdf8,#0ea5e9)', borderRadius: 999 }} />
                </div>
                <span style={{ fontSize: 12, color: '#9fb0c7', minWidth: 60 }}>{qty.toFixed(1)} m³</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #263449' }}>
                {['Batch No', 'Grade', 'Qty (m³)', 'Cement (bags)', 'Water (L)', 'Sand (kg)', 'Aggregate (kg)', 'Operator', 'Date', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '11px 12px', color: '#9fb0c7', fontWeight: 600, textAlign: 'left', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(b => (
                <tr key={b.id} style={{ borderBottom: '1px solid rgba(38,52,73,.4)' }}>
                  <td style={{ padding: '10px 12px', color: '#f7c948', fontWeight: 700, fontFamily: 'monospace' }}>{b.batchNo}</td>
                  <td style={{ padding: '10px 12px' }}><span style={{ padding: '2px 8px', background: 'rgba(56,189,248,.12)', color: '#38bdf8', borderRadius: 999, fontSize: 11, fontWeight: 700 }}>{b.grade}</span></td>
                  <td style={{ padding: '10px 12px', fontWeight: 700 }}>{b.quantity}</td>
                  <td style={{ padding: '10px 12px', color: '#9fb0c7' }}>{b.cementBags ?? '—'}</td>
                  <td style={{ padding: '10px 12px', color: '#9fb0c7' }}>{b.waterLiters ?? '—'}</td>
                  <td style={{ padding: '10px 12px', color: '#9fb0c7' }}>{b.sandKg ?? '—'}</td>
                  <td style={{ padding: '10px 12px', color: '#9fb0c7' }}>{b.aggregateKg ?? '—'}</td>
                  <td style={{ padding: '10px 12px', fontSize: 12 }}>{b.operator || '—'}</td>
                  <td style={{ padding: '10px 12px', color: '#9fb0c7', fontSize: 11 }}>{new Date(b.createdAt).toLocaleDateString('en-IN')}</td>
                  <td style={{ padding: '10px 12px' }}>
                    <div style={{ display: 'flex', gap: 5 }}>
                      <button onClick={() => openEdit(b)} style={{ padding: '4px 8px', background: 'rgba(56,189,248,.1)', border: '1px solid rgba(56,189,248,.2)', borderRadius: 7, cursor: 'pointer', color: '#38bdf8', fontSize: 11 }}>Edit</button>
                      <button onClick={() => remove(b)} style={{ padding: '4px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 7, cursor: 'pointer', color: '#ef4444' }}><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={10} style={{ padding: '30px', textAlign: 'center', color: '#9fb0c7' }}>No batch records found</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div style={{ background: '#0d1930', border: '1px solid #263449', borderRadius: 16, padding: 28, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              {[
                ...
                <div key={k}>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#9fb0c7', marginBottom: 4 }}>{l}</label>
              ...
              <div style={{ gridColumn: '1/-1' }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#9fb0c7', marginBottom: 4 }}>Remarks</label>
