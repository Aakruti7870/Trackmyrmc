...
export default function Orders() {
  ...
  const statusColor = (s: string) => ({ pending: '#f7c948', in_progress: '#38bdf8', completed: '#22c55e', cancelled: '#ef4444' }[s] || '#9fb0c7');
  const statusBg = (s: string) => ({ pending: 'rgba(247,201,72,.12)', in_progress: 'rgba(56,189,248,.12)', completed: 'rgba(34,197,94,.12)', cancelled: 'rgba(239,68,68,.12)' }[s] || 'rgba(159,176,199,.12)');
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#9fb0c7', fontSize: 13 }}>{orders.length} total orders</p>
        </div>
            ...
            <tbody>
              {filtered.map(o => (
                <tr key={o.id} style={{ borderBottom: '1px solid rgba(38,52,73,.4)' }}>
                  <td style={{ padding: '11px 14px', color: '#f7c948', fontWeight: 700, fontFamily: 'monospace' }}>{o.orderNo}</td>
                  <td style={{ padding: '11px 14px', fontWeight: 600 }}>{o.clientName}</td>
                  <td style={{ padding: '11px 14px', color: '#9fb0c7', fontSize: 12 }}>{o.siteName || '—'}</td>
                  <td style={{ padding: '11px 14px' }}><span style={{ padding: '2px 8px', background: 'rgba(56,189,248,.12)', color: '#38bdf8', borderRadius: 999, fontSize: 11, fontWeight: 700 }}>{o.grade}</span></td>
                  <td style={{ padding: '11px 14px', fontWeight: 700 }}>{o.quantity}</td>
                  <td style={{ padding: '11px 14px', color: o.pumpRequired ? '#22c55e' : '#9fb0c7', fontSize: 12 }}>{o.pumpRequired ? 'Yes' : 'No'}</td>
                  <td style={{ padding: '11px 14px', color: '#9fb0c7', fontSize: 12 }}>{o.deliveryDate || '—'}</td>
                  ...
                  <td style={{ padding: '11px 14px' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {o.status === 'pending' && (
                        <Link href="/dispatch">
                          <button style={{ padding: '4px 10px', background: 'rgba(34,197,94,.1)', border: '1px solid rgba(34,197,94,.2)', borderRadius: 7, cursor: 'pointer', color: '#22c55e', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
                            <Truck size={11} /> Dispatch
                          </button>
                        </Link>
                      )}
                      <button onClick={() => openEdit(o)} style={{ padding: '4px 8px', background: 'rgba(56,189,248,.1)', border: '1px solid rgba(56,189,248,.2)', borderRadius: 7, cursor: 'pointer', color: '#38bdf8' }}><Edit2 size={12} /></button>
                      <button onClick={() => remove(o)} style={{ padding: '4px 8px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 7, cursor: 'pointer', color: '#ef4444' }}><Trash2 size={12} /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan={9} style={{ padding: '30px', textAlign: 'center', color: '#9fb0c7' }}>No orders found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#0d1930', border: '1px solid #263449', borderRadius: 16, padding: 28, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#9fb0c7', marginBottom: 4 }}>Delivery Date</label>
                <input type="date" value={form.deliveryDate || ''} onChange={e => setForm(f => ({ ...f, deliveryDate: e.target.value }))} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#9fb0c7', marginBottom: 4 }}>Delivery Time</label>
                <input type="time" value={form.deliveryTime || ''} onChange={e => setForm(f => ({ ...f, deliveryTime: e.target.value }))} style={inputStyle} />
              </div>
              {modal === 'edit' && (
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#9fb0c7', marginBottom: 4 }}>Status</label>
