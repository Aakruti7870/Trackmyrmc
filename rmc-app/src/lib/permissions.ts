...
const ADMIN_PATHS = ['/', '/orders', '/dispatch', '/clients', '/vehicles', '/drivers', '/batch-report', '/batch-sheets', '/attendance', '/live-drivers', '/mix-design', '/reports', '/forecast', '/freshness', '/challans', '/shift-report', '/recurring', '/fuel-log', '/expense-review', '/emergencies', '/plants', '/users', '/user-management', '/activity-log', '/audit-log', '/automations', '/kyc', '/kyc-admin', '/profile'];

export const ROLE_ALLOWED_PATHS: Record<Role, string[]> = {
  // RMC Plant Network contains statewide imports and cross-tenant onboarding and
  // is therefore reserved for authority (the existing Super Admin role).
  authority:      ['/command', '/whatsapp', '/kiosk', '/rmc-plant-network', ...ADMIN_PATHS],
  ...
  client:         ['/my-orders', '/nearby-plants', '/challans', '/kyc', '/profile'],
...
export function setPermissionOverrides(overrides: Partial<Record<string, string[]>> | null | undefined): void {
  ...
  if (next.authority) {
    for (const required of ['/profile', '/command', '/rmc-plant-network']) {
