...
describe('Android release metadata — v1.30 / versionCode 31', () => {
  it('build.gradle declares versionCode 31 and versionName "1.30"', () => {
    const gradle = readFileSync(resolve(rmcAppRoot, 'android/app/build.gradle'), 'utf8');
    expect(gradle).toMatch(/versionCode\s+31\b/);
    expect(gradle).toMatch(/versionName\s+"1\.30"/);
  ...
  it('the signed release workflow uploads a v1.30 / vc31-named artifact', () => {
    const signed = readFileSync(resolve(repoRoot, '.github/workflows/android-signed-release.yml'), 'utf8');
    expect(signed).toContain('TrackMyRMC-v1.30-vc31-signed-play-console');
  });
});
