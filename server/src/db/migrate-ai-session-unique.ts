...
export interface AiSessionUniqueMigrationResult {
...
export async function migrateAiSessionUnique(
  db: DB,
  log: (msg: string) => void = () => {},
): Promise<AiSessionUniqueMigrationResult> {
  ...
  if (!hadNew) {
    await db.execute(
      sql`CREATE UNIQUE INDEX IF NOT EXISTS ${sql.identifier(NEW_INDEX)} ON ai_conversations (session_id, user_id)`,
    );
  ...
  const result: AiSessionUniqueMigrationResult = {
