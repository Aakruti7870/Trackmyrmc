...
import sponsoredNearbyRoutes from './routes/sponsoredNearby.js';
...
if (!Number.isInteger(PORT) || PORT < 1 || PORT > 65535) throw new Error(`Invalid PORT value: ${JSON.stringify(rawPort)} — must be an integer between 1 and 65535`);

const STATIC_ALLOWED_ORIGINS = new Set<string>(['https://localhost','http://localhost','capacitor://localhost','ionic://localhost','https://www.goldetech.com','https://goldetech.com','https://www.trackmyrmc.com','https://trackmyrmc.com']);
for (const origin of (process.env.CORS_ALLOWED_ORIGINS || '').split(',').map(s => s.trim()).filter(Boolean)) STATIC_ALLOWED_ORIGINS.add(origin);
if (process.env.REPLIT_DEV_DOMAIN) STATIC_ALLOWED_ORIGINS.add(`https://${process.env.REPLIT_DEV_DOMAIN}`);
...
app.use('/api/plants', sponsoredNearbyRoutes);
...
async function tickRecurringOrders() { if (recurringRunning) return; recurringRunning = true; try { const count = await runDueRecurringOrders(); if (count > 0) console.log(`Recurring scheduler created ${count} order(s)`); } catch (error) { console.error('Recurring scheduler failed', error); } finally { recurringRunning = false; } }
let whatsappRetryRunning = false;
async function tickWhatsAppRetries() { if (whatsappRetryRunning) return; whatsappRetryRunning = true; try { const result = await runDueWhatsAppRetries(); if (result.sent > 0 || result.gaveUp > 0) console.log(`WhatsApp retry: ${result.sent} sent, ${result.retried} rescheduled, ${result.gaveUp} gave up`); } catch (error) { console.error('WhatsApp retry tick failed', error); } finally { whatsappRetryRunning = false; } }
let freshnessRunning = false;
async function tickFreshness() { if (freshnessRunning) return; freshnessRunning = true; try { await tickFreshnessAlerts(); } catch (error) { console.error('Freshness alert tick failed', error); } finally { freshnessRunning = false; } }
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  cleanupOldAttempts().catch(() => {}); setInterval(() => cleanupOldAttempts().catch(() => {}), 60 * 60 * 1000);
  void tickRecurringOrders(); setInterval(() => void tickRecurringOrders(), 60 * 60 * 1000);
