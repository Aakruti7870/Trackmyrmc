...
const STATIC_ALLOWED_ORIGINS = new Set<string>([
  ...
  'https://www.goldetech.com', 'https://goldetech.com',
  'https://www.trackmyrmc.com', 'https://trackmyrmc.com',
]);
...
async function tickRecurringOrders() {
  ...
  try {
    ...
    if (count > 0) console.log(`Recurring scheduler created ${count} order(s)`);
  } catch (error) { console.error('Recurring scheduler failed', error); }
...
async function tickWhatsAppRetries() {
  ...
  try {
    ...
    if (result.sent > 0 || result.gaveUp > 0) console.log(`WhatsApp retry: ${result.sent} sent, ${result.retried} rescheduled, ${result.gaveUp} gave up`);
  } catch (error) { console.error('WhatsApp retry tick failed', error); }
  finally { whatsappRetryRunning = false; }
}
let freshnessRunning = false;
async function tickFreshness() {
  if (freshnessRunning) return;
  freshnessRunning = true;
  try { await tickFreshnessAlerts(); }
  catch (error) { console.error('Freshness alert tick failed', error); }
...
async function tickDiscoveryCleanup() {
  ...
  try { await Promise.all([cleanupExpiredRateLimits(), cleanupExpiredCache()]); }
  catch (error) { console.error('Discovery cleanup tick failed', error); }
  finally { discoveryCleanupRunning = false; }
}

app.listen(PORT, '0.0.0.0', () => {
  console.log(`TrackMyRMC API running on port ${PORT}`);
  // Cloud Run must be able to probe /api/health immediately. Runtime recovery
  // and database-backed initialization are best-effort background work and
  // must never delay binding the injected PORT.
  syncSmtpFromEnv().catch(error => console.error('syncSmtpFromEnv failed', error));
