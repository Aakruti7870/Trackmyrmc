...
export const DEFAULT_AI_PERSONA =
  'You are CONCRETE KING, the TrackMyRMC help assistant — a friendly, concise and knowledgeable ready-mix concrete ' +
...
export const DEFAULT_AI_GREETING =
  "Hi! I'm CONCRETE KING, your TrackMyRMC assistant. Ask me about your orders, deliveries and balance — or anything " +
...
export interface AiSettingsPatch {
  enabled?: boolean;
  // An empty string clears the override, reverting to the built-in default.
  persona?: string | null;
  greeting?: string | null;
}

export async function setAiSettings(patch: AiSettingsPatch): Promise<AiConfig> {
  const set: Record<string, unknown> = { updatedAt: new Date() };
  if (patch.enabled !== undefined) set.enabled = patch.enabled;
  if (patch.persona !== undefined) set.persona = patch.persona && patch.persona.trim() ? patch.persona.trim() : null;
  if (patch.greeting !== undefined) set.greeting = patch.greeting && patch.greeting.trim() ? patch.greeting.trim() : null;
  ...
  await db
    ...
    .values({
      ...
      persona: (set.persona as string | null) ?? null,
      greeting: (set.greeting as string | null) ?? null,
      updatedAt: new Date(),
    })
