...
import {
  users, clients, orders, challans, ledgerEntries, plants, plantCustomers, vehicles,
} from '../db/schema.js';
...
// Topics the agent can be asked about. Each retrieval function declares the
// topic it serves and the roles allowed to call it.
export type Topic =
  | 'orders' | 'deliveries' | 'trips' | 'ledger' | 'clients'
  | 'fleet' | 'production' | 'plant_info' | 'reports';

const PLANT_STAFF_ROLES = ['plant_owner', 'admin', 'supervisor', 'dispatcher', 'plant_operator', 'authority'];

export interface RetrievalCtx {
  user: AuthPayload;
  // The resolved plant scope for plant-facing reads. For plant-bound staff this
  // is always their own user.plantId; for platform staff it is the validated
  // selectedPlantId (or null when none chosen → no private plant data).
  scopePlantId: number | null;
}
...
interface RetrievalFn {
  ...
  topic: Topic;
  roles: string[];
  run: (ctx: RetrievalCtx) => Promise<string | null>;
}
...
const RETRIEVALS: RetrievalFn[] = [
  // ----- customer -----
  {
    ...
    run: async ({ user }) => {
      ...
      return 'Your recent orders:\n' + rows.map(r =>
        `- ${r.orderNo}: ${r.grade}, ${r.quantity} m³, status ${r.status}` +
        `${r.plantName ? `, plant ${r.plantName}` : ''}${r.deliveryDate ? `, delivery ${r.deliveryDate}` : ''}`,
      ).join('\n');
    },
  },
  {
    ...
    run: async ({ user }) => {
      ...
      return 'Your recent deliveries (challans):\n' + rows.map(r =>
        `- ${r.challanNo}: ${r.grade}, ${r.quantity} m³, status ${r.status}` +
        `${r.dispatchTime ? `, dispatched ${new Date(r.dispatchTime).toLocaleString('en-IN')}` : ''}` +
        `${r.deliveryTime ? `, delivered ${new Date(r.deliveryTime).toLocaleString('en-IN')}` : ''}`,
      ).join('\n');
    },
  },
  {
    ...
    run: async ({ user }) => {
      ...
      if (recent.length) {
        out += '\nRecent ledger entries:\n' + recent.map(e =>
          `- ${e.type} ${money(e.amount)} — ${e.description}`).join('\n');
      }
      return out;
    },
  },
  ...
  // ----- staff / platform (plant-scoped) -----
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      if (!scope) return null; // platform staff with no plant selected → no private data
      const rows = await db.select({
        orderNo: orders.orderNo, grade: orders.grade, quantity: orders.quantity,
      ...
      return "Your plant's recent orders:\n" + rows.map(r =>
        `- ${r.orderNo}: ${r.grade}, ${r.quantity} m³, status ${r.status}` +
        `${r.customerCode ? `, customer ${r.customerCode}` : ''}`,
      ).join('\n');
    },
  },
  {
  ...
  },
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      const rows = await db.select({
        ...
        total: sql<string>`COALESCE(SUM(${challans.quantity}), 0)`,
        loads: sql<number>`COUNT(*)`,
      }).from(challans).where(scope).groupBy(challans.grade);
      if (rows.length === 0) return null;
      return "Your plant's production/dispatch by grade:\n" + rows.map(r =>
        `- ${r.grade}: ${parseFloat(r.total).toLocaleString('en-IN')} m³ across ${r.loads} load(s)`,
      ).join('\n');
    },
  },
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      return "Your plant's customers:\n" + rows.map(r =>
        `- ${r.customerCode ?? '(no code)'}: contact ${r.contactPerson}, outstanding ${money(r.outstandingAmount)}`,
      ).join('\n');
    },
  },
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      const [agg] = await db.select({
        totalOutstanding: sql<string>`COALESCE(SUM(${clients.outstandingAmount}), 0)`,
  ...
  },
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      if (rows.length === 0) return null;
      const counts = rows.reduce<Record<string, number>>((m, v) => { m[v.status] = (m[v.status] ?? 0) + 1; return m; }, {});
      const summary = Object.entries(counts).map(([s, n]) => `${n} ${s}`).join(', ');
      return `Your plant's fleet (transit-mixer pool) — ${summary}:\n` + rows.map(v =>
        `- ${v.vehicleNo} (${v.type}): ${v.status}`).join('\n');
    },
  },
  // ----- everyone: public plant info -----
  {
    ...
    run: async ({ scopePlantId }) => {
      ...
      const [agg] = await db.select({ approved: sql<number>`COUNT(*)` })
...
function detectTopics(message: string): Topic[] {
  const found: Topic[] = [];
  for (const [topic, re] of Object.entries(TOPIC_KEYWORDS) as [Topic, RegExp][]) {
    if (re.test(message)) found.push(topic);
  }
  return found;
}
...
function defaultTopics(role: string): Topic[] {
  if (role === 'client') return ['orders', 'deliveries', 'ledger'];
  if (role === 'driver') return ['trips'];
  return ['orders', 'deliveries'];
}

function allowedRetrievalsForTopic(role: string, topic: Topic): RetrievalFn[] {
...
function topicExistsForAnyRole(topic: Topic): boolean {
  return RETRIEVALS.some(r => r.topic === topic);
}

export interface BuiltContext {
  // True when the user explicitly asked for something their role may never see;
  // the route short-circuits with REFUSAL and never calls the model.
  refused: boolean;
  context: string;
...
export async function resolveScopePlantId(
  user: AuthPayload,
  ...
  if (selectedPlantId == null || selectedPlantId === '') return null;
...
export async function buildAiContext(
  ...
  const ctx: RetrievalCtx = { user, scopePlantId };
  const blocks: string[] = [];
