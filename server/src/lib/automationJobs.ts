...
const STAFF_ALERT_ROLES = ['admin', 'plant_owner', 'dispatcher', 'authority'] as const;
...
function num(cfg: AutomationConfig, key: string, fallback: number): number {
  const v = cfg[key];
  return typeof v === 'number' && Number.isFinite(v) ? v : fallback;
}

function bool(cfg: AutomationConfig, key: string): boolean {
  return cfg[key] === true;
}

function str(cfg: AutomationConfig, key: string): string {
  const v = cfg[key];
...
async function recordAutomationWhatsApp(
  automation: AutomationName,
  link: { orderId?: number | null; challanId?: number | null; plantId?: number | null },
  toPhone: string | null,
  result: Awaited<ReturnType<typeof sendWhatsAppWithRetry>>,
): Promise<void> {
  try {
...
async function plantStaffEmails(plantId: number | null, roles: readonly string[]): Promise<string[]> {
...
export async function runOrderReminders(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const o of rows) {
    ...
    if (ist.hour < num(eff.config, 'sendHour', 18)) continue;
    if (!(await claimSend('orderReminders', 'order', o.id, `d:${o.deliveryDate}`, o.plantId))) continue;
    sent += 1;
...
export async function runDeliveryFollowUps(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const c of rows) {
    ...
    if (!c.dispatchTime || now.getTime() - c.dispatchTime.getTime() < delayMs) continue;
    if (!(await claimSend('deliveryFollowUp', 'challan', c.id, 'overdue', c.plantId))) continue;
    sent += 1;
...
export async function maybeSendTripShare(challanId: number): Promise<void> {
  try {
    ...
    if (!(await claimSend('tripShare', 'challan', row.id, 'share', row.plantId))) return;
...
export async function runDigests(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const plant of plantRows) {
    ...
    if (!eff.enabled) continue;
    if (ist.hour < num(eff.config, 'sendHour', 8)) continue;

    const weekly = eff.config.frequency === 'weekly';
    if (weekly && ist.weekday !== num(eff.config, 'weekday', 1)) continue;
    ...
    if (!(await claimSend('digest', 'plant', plant.id, windowKey, plant.id))) continue;

    const inPeriod = and(
      eq(challans.plantId, plant.id),
      sql`${challans.createdAt} >= ${periodStart}`,
      sql`${challans.createdAt} < ${periodEnd}`,
    );
    const [challanAgg] = await db
      .select({
        trips: sql<number>`count(*)::int`,
        delivered: sql<number>`count(*) filter (where ${challans.status} = 'delivered')::int`,
        volume: sql<number>`coalesce(sum(coalesce(${challans.deliveredQuantity}, ${challans.quantity})::numeric), 0)`,
      })
      .from(challans)
      .where(inPeriod);
...
export async function runFuelAnomalyAlerts(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const plant of plantRows) {
    ...
    let rows: Awaited<ReturnType<typeof computeFuelReconciliation>>['rows'];
    try {
    ...
    for (const r of rows) {
      if (r.variancePct == null || r.variancePct <= thresholdPct) continue;
      const key = periodKey(ist.dayIndex, windowDays);
      if (!(await claimSend('fuelAnomaly', 'vehicle', r.vehicleId, key, plant.id, `${r.variancePct}%`))) continue;
      sent += 1;
...
export async function runWinBacks(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const c of rows) {
    ...
    if (!c.lastOrderAt || now.getTime() - c.lastOrderAt.getTime() < inactiveDays * DAY_MS) continue;
    if (!(await claimSend('winBack', 'client', c.id, periodKey(ist.dayIndex, inactiveDays), c.plantId))) continue;
    sent += 1;
...
export async function runIdleVehicleAlerts(snapshot: AutomationSettingsSnapshot, now = new Date(), scopePlantId?: number): Promise<number> {
  ...
  for (const v of rows) {
    ...
    if (!reference || now.getTime() - reference.getTime() < idleDays * DAY_MS) continue;
    if (!(await claimSend('idleVehicle', 'vehicle', v.id, periodKey(ist.dayIndex, idleDays), v.plantId))) continue;
    sent += 1;
...
// ---- 8. Auto-cleanup ------------------------------------------------------------------------
// Safe housekeeping, ON by default. Purges rows that are unquestionably dead:
// expired OTP codes and rate-limit/cache entries, long-expired one-time tokens,
// and trashed (soft-deleted) accounts past the retention window. The cleanup
// setting is platform-wide (plant overrides are ignored — none of this data is
// meaningfully plant-scoped).
export interface CleanupResult {
  otpCodes: number;
  staffOtpCodes: number;
...
export async function runCleanup(snapshot: AutomationSettingsSnapshot, now = new Date()): Promise<CleanupResult | null> {
  ...
  result.passwordTokens = (await db.delete(passwordSetupTokens).where(
    sql`coalesce(${passwordSetupTokens.usedAt}, ${passwordSetupTokens.expiresAt}) < ${retentionCutoff}`,
  ).returning({ id: passwordSetupTokens.id })).length;
  result.trackingTokens = (await db.delete(trackingTokens).where(
    sql`coalesce(${trackingTokens.revokedAt}, ${trackingTokens.expiresAt}) < ${retentionCutoff}`,
  ).returning({ id: trackingTokens.id })).length;
  ...
  for (const u of trashed) {
    ...
    if (!(await claimSend('cleanup', 'user', u.id, 'purge'))) continue;
