...
export function normalizeCommissionPct(value: unknown): number | null {
...
export async function setPlatformCommissionPct(value: number | null): Promise<void> {
...
export function effectiveCommissionPct(
  plant: { commissionPct?: string | null },
  globalPct: number,
): number {
  if (plant.commissionPct == null || plant.commissionPct === '') return globalPct;
