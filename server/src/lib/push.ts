...
function ensureConfigured(): boolean {
  ...
  if (!publicKey || !privateKey) {
    console.warn(
      '[push] VAPID keys not configured (VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY missing). ' +
      'Web push disabled.',
    );
    configured = false;
    return false;
...
export async function deleteSubscription(endpoint: string, userId?: number): Promise<void> {
...
export async function sendPushToClientUsers(clientId: number, payload: PushPayload): Promise<number> {
  if (!ensureConfigured()) return 0;
  const linked = await db
    .select({ id: users.id })
    .from(users)
    .where(
      and(eq(users.linkedClientId, clientId), eq(users.isActive, true), isNull(users.deletedAt)),
    );
...
export async function sendPushToPlantAndPlatformStaff(
  plantId: number | null,
  roles: readonly string[],
  payload: PushPayload,
): Promise<number> {
...
export async function plantAndPlatformStaffIds(
  plantId: number | null,
  roles: readonly string[],
): Promise<number[]> {
  const roleVals = [...roles] as (typeof users.role.enumValues)[number][];
