...
export type AutomationName = (typeof AUTOMATIONS)[number];

export function isAutomationName(value: string): value is AutomationName {
  return (AUTOMATIONS as readonly string[]).includes(value);
}

// Per-automation tuning knobs. Kept flat (numbers/booleans/strings only) so the
// admin UI can render them generically and zod-style clamping stays trivial.
...
export const AUTOMATION_DEFS: Record<AutomationName, AutomationDefinition> = {
  orderReminders: {
    name: 'orderReminders',
...
export interface EffectiveAutomation {
  ...
  /** 'plant' when a plant override row decided `enabled`, 'global' when the
   *  platform row did, 'default' when no row exists. */
...
function mergeConfig(def: AutomationDefinition, ...layers: (unknown | null)[]): AutomationConfig {
  const out: AutomationConfig = { ...def.defaultConfig };
  for (const layer of layers) {
    if (!layer || typeof layer !== 'object') continue;
    for (const [key, defVal] of Object.entries(def.defaultConfig)) {
      const val = (layer as Record<string, unknown>)[key];
      if (val === undefined || val === null) continue;
      if (typeof val === typeof defVal) out[key] = val as number | boolean | string;
    }
  }
  return out;
}

type SettingsRow = { automation: string; plantId: number; enabled: boolean; config: unknown };

function resolveEffective(
  name: AutomationName,
  globalRow: SettingsRow | undefined,
  plantRow: SettingsRow | undefined,
): EffectiveAutomation {
...
export class AutomationSettingsSnapshot {
  ...
  constructor(rows: SettingsRow[]) {
    this.rows = rows;
  }

  effective(name: AutomationName, plantId: number | null | undefined): EffectiveAutomation {
  ...
  enabledAnywhere(name: AutomationName): boolean {
    const def = AUTOMATION_DEFS[name];
...
export async function loadAutomationSnapshot(): Promise<AutomationSettingsSnapshot> {
  const rows = await db
    .select({
...
export async function getEffectiveAutomation(
  name: AutomationName,
  plantId: number | null | undefined,
): Promise<EffectiveAutomation> {
  ...
  return resolveEffective(
    name,
    rows.find(r => r.plantId === GLOBAL_SCOPE),
    plantId ? rows.find(r => r.plantId === plantId) : undefined,
  );
}
...
export async function lastSentAt(plantId: number | null | undefined): Promise<Record<string, string | null>> {
  const scope = plantId
  ...
  const rows = await db
    .select({
      ...
      last: sql<string | null>`max(${automationSends.sentAt})`,
    })
...
export async function getLastRunMap(): Promise<Record<string, string>> {
...
export async function recordLastRun(names: AutomationName[], when = new Date()): Promise<void> {
  try {
    const map = await getLastRunMap();
    for (const n of names) map[n] = when.toISOString();
...
export interface IstParts {
  ...
  dateStr: string;
  /** YYYY-MM-DD of the IST day after `dateStr` */
  tomorrowStr: string;
  hour: number;
  /** 0=Sunday … 6=Saturday, in IST */
  weekday: number;
  /** Whole days since the Unix epoch, in IST — drives period-index windows. */
  dayIndex: number;
}

export function istParts(now = new Date()): IstParts {
...
// dedupes them.
export function periodKey(dayIndex: number, windowDays: number): string {
...
export function sanitizeConfig(name: AutomationName, input: unknown): AutomationConfig {
  const def = AUTOMATION_DEFS[name];
  ...
  for (const [key, defVal] of Object.entries(def.defaultConfig)) {
    ...
    if (typeof defVal === 'boolean') {
      ...
      switch (key) {
        case 'sendHour': out[key] = clampNumber(val, 0, 23, defVal); break;
        case 'weekday': out[key] = clampNumber(val, 0, 6, defVal); break;
        case 'delayHours': out[key] = clampNumber(val, 1, 72, defVal); break;
        case 'ttlHours': out[key] = clampNumber(val, 1, 168, defVal); break;
        case 'thresholdPct': out[key] = clampNumber(val, 1, 500, defVal); break;
        case 'windowDays': out[key] = clampNumber(val, 1, 90, defVal); break;
        case 'inactiveDays': out[key] = clampNumber(val, 3, 365, defVal); break;
        case 'idleDays': out[key] = clampNumber(val, 1, 90, defVal); break;
        case 'retentionDays': out[key] = clampNumber(val, 7, 365, defVal); break;
        default: out[key] = clampNumber(val, 0, 100000, defVal);
      }
    } else {
      // strings: template names etc. — trim and cap length.
      if (typeof val === 'string') out[key] = val.trim().slice(0, 200);
    }
  }
