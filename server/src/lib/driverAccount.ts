...
export type DriverLoginResult =
  // A live, active driver matched this number — sign them in as role 'driver'.
  | {
      kind: 'match';
      user: UserRow;
      driverId: number;
  ...
  // Matched a driver row but the login can't proceed (disabled / conflicting link).
  | { kind: 'error'; status: number; error: string }
  // No active driver holds this number — the caller should fall through to the
  // customer resolver.
  | { kind: 'none' };
 ...
 */
export async function resolveDriverByPhone(phone: string): Promise<DriverLoginResult> {
  const last10 = phone.replace(/\D/g, '').slice(-10);
  // Too short to be a real mobile number — let the customer path handle it.
  if (last10.length !== 10) return { kind: 'none' };
  ...
  const [driver] = await db
    ...
    .where(
      and(
        ...
        sql`right(regexp_replace(${drivers.phone}, '[^0-9]', '', 'g'), 10) = ${last10}`,
      ),
    )
    .orderBy(drivers.id)
    .limit(1);
  if (!driver) return { kind: 'none' };
