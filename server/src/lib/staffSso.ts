...
export const SSO_ROLES = ['admin', 'dispatcher', 'plant_operator', 'authority', 'plant_owner', 'supervisor'] as const;

type UserRow = typeof users.$inferSelect;

export type StaffSsoResult =
 ...
 */
export async function resolveStaffSsoUser(email: string): Promise<StaffSsoResult> {
  ...
  if (!user || !user.isActive || user.deletedAt) return notAuthorized;
  if (!(SSO_ROLES as readonly string[]).includes(user.role)) return notAuthorized;
