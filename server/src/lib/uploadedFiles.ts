...
export type UploadedFileType = 'proof_photo' | 'fuel_bill';
...
export interface RegisterFileInput {
  ...
  accessLevel?: UploadedFileAccess;
}
...
export async function registerUploadedFile(input: RegisterFileInput) {
...
export async function registerUploadedFileSafe(input: RegisterFileInput): Promise<void> {
  try {
...
export async function registerUploadedFilesSafe(inputs: RegisterFileInput[]): Promise<void> {
...
export async function unregisterUploadedFilesSafe(storagePaths: string[]): Promise<void> {
  if (!storagePaths.length) return;
  try {
    await db.delete(uploadedFiles).where(
      inArray(uploadedFiles.storagePath, storagePaths),
    );
  } catch (err) {
...
export async function getStorageUsage(): Promise<StorageUsageRow[]> {
  const rows = await db
    .select({
...
export async function listPlantFiles(plantId: number, fileType?: string) {
  const conds = [eq(uploadedFiles.plantId, plantId)];
  if (fileType) conds.push(eq(uploadedFiles.fileType, fileType));
  return db
    .select()
    .from(uploadedFiles)
