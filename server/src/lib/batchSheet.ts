...
// else 1 decimal.
export const roundWeight = (v: number) => round(v, v < 10 ? 2 : 1);
...
// plant = 6 × 1.0 + 1 × 0.5).
export function computeBatchSizes(productionQty: number, batchSize: number): number[] {
  ...
  if (rows === 0) throw new Error('Quantity too small for one batch');
  const sizes = Array.from({ length: full }, () => batchSize);
  if (remainder > 0.0001) sizes.push(remainder);
  return sizes;
}
...
export function generateActuals(
...
export function computeTotals(materials: RecipeMaterial[], batches: BatchRow[]): SheetTotals {
  let totalSetMass = 0;
...
export function parseMaterials(input: unknown): RecipeMaterial[] {
  if (!Array.isArray(input) || input.length === 0) {
    throw new Error('At least one material is required');
  }
  if (input.length > 20) throw new Error('Too many materials (max 20)');
  const seen = new Set<string>();
  return input.map((raw, i) => {
    const r = raw as Record<string, unknown>;
    const label = typeof r.label === 'string' ? r.label.trim() : '';
    if (!label) throw new Error(`Material ${i + 1}: name is required`);
    const key = (typeof r.key === 'string' && r.key.trim())
      ? r.key.trim()
      : label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || `m${i + 1}`;
    if (seen.has(key)) throw new Error(`Material "${label}" is duplicated`);
    seen.add(key);
...
export function parseBatches(input: unknown, materials: RecipeMaterial[]): BatchRow[] {
  if (!Array.isArray(input) || input.length === 0) throw new Error('At least one batch row is required');
  if (input.length > MAX_BATCH_ROWS) throw new Error(`Too many batch rows (max ${MAX_BATCH_ROWS})`);
  return input.map((raw, i) => {
    const r = raw as Record<string, unknown>;
    ...
    const src = (r.actuals && typeof r.actuals === 'object') ? r.actuals as Record<string, unknown> : {};
    const actuals: Record<string, number> = {};
    for (const m of materials) {
