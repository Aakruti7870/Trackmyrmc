...
// Stub the outbound email channel so the jobs never touch real SMTP and we can
// observe exactly who each run tried to reach. Must be registered before the
...
async function seedReminderTarget(plantName: string, clientEmail: string) {
...
async function enableOrderRemindersEverywhere() {
  await saveAutomationSettings('orderReminders', null, true,
    sanitizeConfig('orderReminders', { sendHour: 0, email: true, push: false, whatsapp: false }));
}
