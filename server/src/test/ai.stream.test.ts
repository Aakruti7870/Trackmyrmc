...
async function createUser(role: string, email: string, opts: { plantId?: number | null; linkedClientId?: number | null } = {}) {
...
async function createClient(plantId: number, code: string) {
...
async function createOrder(plantId: number, clientId: number, orderNo: string) {
...
function tokenFor(u: { id: number; email: string; role: string; name: string; plantId?: number | null; linkedClientId?: number | null }) {
  return signToken({
...
function stream(token: string | null, body: Record<string, unknown>) {
