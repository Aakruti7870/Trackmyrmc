...
async function createVehicle(vehicleNo: string, opts: { mileageKmpl?: string; idleBurnLph?: string } = {}) {
...
async function createChallan(opts: {
  vehicleId: number; clientId: number; driverId?: number;
  odometerStart?: number; odometerEnd?: number;
  siteArrivalTime?: Date; siteReleaseTime?: Date;
}) {
  challanSeq += 1;
...
test('reconciliation flags a vehicle whose actual diesel exceeds expected beyond the variance threshold', async () => {
  ...
  const row = res.body.rows.find((r: { vehicleId: number }) => r.vehicleId === vehicle.id);
