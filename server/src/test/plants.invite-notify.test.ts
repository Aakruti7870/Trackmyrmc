...
test('extra recipients are emailed on top of the role-based audience', async () => {
  ...
  assert.deepEqual(
    [...inviteCalls[0].emails].sort(),
    ['admin@test.com', 'onboarding@team.com'].sort(),
    'the admin row AND the extra mailbox are both notified',
  );
});
...
test('clearing the roles routes the email to only the explicit recipients', async () => {
  ...
  assert.equal(inviteCalls.length, 1, 'the mailer is invoked once');
  assert.deepEqual(
    [...inviteCalls[0].emails],
    ['shared@team.com'],
    'with no roles selected only the explicit mailbox is notified — NOT the admin row',
  );
});
...
test('a chosen role resolves to its active members for both the toast and the email', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      [...inviteCalls[0].emails],
      [dispatcher.email],
      'only the active dispatcher is notified (the deactivated one is skipped)',
    );
  } finally {
    adminSSE.close(); dispatcherSSE.close();
  }
});
