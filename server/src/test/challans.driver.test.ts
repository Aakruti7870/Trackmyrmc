...
async function createWriteRoleUser(email: string) {
  const passwordHash = await bcrypt.hash(PASSWORD, 10);
  const [user] = await db.insert(users).values({
...
test('a driver PUT replaces an existing proof photo with a different one; hasProofPhoto stays true', async (t) => {
  ...
  const rows = await db.select({ photo: challanProofPhotos.photo })
    .from(challanProofPhotos).where(eq(challanProofPhotos.challanId, challan.id));
  assert.deepEqual(rows.map(r => r.photo), ['/objects/uploads/replace-1'],
    'old photo is gone, new entity path is stored');
  ...
  const detail = await request(app)
    ...
    .set('Authorization', `Bearer ${tokenFor(user)}`);
  assert.equal(detail.status, 200);
  assert.deepEqual(detail.body.proofPhotos, ['/objects/uploads/replace-1#signed'],
    'detail returns the resolved signed URL for the replacement photo');
