...
type FailureAlertCall = { emails: string[]; details: Record<string, unknown> };
let failureAlerts: FailureAlertCall[] = [];
let emailsSent: { kind: 'delivery' | 'order'; to: string }[] = [];
...
mock.module('../lib/email.js', {
  namedExports: {
    ...
    sendWhatsAppFailureAlertEmail: async (emails: string[], details: Record<string, unknown>) => {
      failureAlerts.push({ emails, details });
      return true;
    },
    sendOrderPlacedEmail: async (to: string) => {
...
let seq = 0;
async function seedChallan(opts: { phone?: string | null; withSite?: boolean; email?: string | null } = {}) {
...
async function seedOrder(opts: { phone?: string | null; email?: string | null } = {}) {
  const [client] = await db.insert(clients).values({
    name: 'Acme Co', contactPerson: 'Jane',
    phone: opts.phone === undefined ? '9991112222' : (opts.phone as string),
    email: opts.email ?? null,
...
async function seedStaff(role: string, email: string, opts: { active?: boolean } = {}) {
