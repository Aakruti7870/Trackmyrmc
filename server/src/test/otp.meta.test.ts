...
const TWILIO_VERIFY_KEYS = ['TWILIO_ACCOUNT_SID', 'TWILIO_AUTH_TOKEN', 'TWILIO_VERIFY_SERVICE_SID'] as const;
const saved: Record<string, string | undefined> = {};
const realFetch = global.fetch;

before(() => {
  for (const k of [...META_KEYS, ...TWILIO_VERIFY_KEYS]) saved[k] = process.env[k];
  // Force the "Meta configured, Twilio Verify absent" combination.
  for (const k of TWILIO_VERIFY_KEYS) delete process.env[k];
...
test('sendOtp delivers via the Meta auth template and verifies locally', async () => {
  type Call = { url: string; body: { template: { name: string; components: { parameters: { text: string }[] }[] } } };
  const calls: Call[] = [];
  global.fetch = (async (url: unknown, init: { body: string }) => {
    calls.push({ url: String(url), body: JSON.parse(init.body) as Call['body'] });
...
test('a failed Meta send removes the stored code', async () => {
  global.fetch = (async () => new Response('bad template name', { status: 400 })) as unknown as typeof fetch;
