...
let ownerInviteCalls: Array<{ email: string; name: string }> = [];
...
mock.module('../lib/email.js', {
  namedExports: {
    ...
    sendOwnerInviteEmail: async (email: string, name: string) => {
...
async function createPlant(opts: { name: string; networkStatus?: string; showOnNetwork?: boolean }) {
...
test('platform admin send-invite provisions an owner and moves the plant pending→invited', async () => {
  ...
  assert.ok(typeof res.body.inviteUrl === 'string' && res.body.inviteUrl.includes('/set-password?token='),
    'always returns a usable invite link');
