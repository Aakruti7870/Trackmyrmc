...
// Stub the outbound email channel with a switchable failure mode. Must be
// registered before the unit under test is loaded.
type EmailCall = { to: string[]; subject: string };
...
mock.module('../lib/automationEmail.js', {
  namedExports: {
    sendAutomationEmail: async (msg: EmailCall) => {
      if (failEmails) throw new Error('SMTP exploded (test)');
      emails.push(msg);
      return true;
    },
  },
...
const {
  users, plants, clients, orders, challans, vehicles, automationSends,
} = await import('../db/schema.js');
const { signToken } = await import('../middleware/auth.js');
const { hashPassword } = await import('../lib/password.js');
const {
  getLastRunMap, loadAutomationSnapshot, sanitizeConfig, saveAutomationSettings,
} = await import('../lib/automations.js');
...
async function enable(name: AutomationName, config: Record<string, unknown>) {
...
async function seedBusyPlant(now = NOW) {
  const hAgo = (n: number) => new Date(now.getTime() - n * 60 * 60 * 1000);
  const dAgo = (n: number) => new Date(now.getTime() - n * DAY_MS);
...
async function claimedAutomations(): Promise<string[]> {
...
test('a crashing job does not stop the rest of the tick, and the failure shows in last-run metadata', async () => {
  ...
  try {
    ...
    assert.ok(
      errorLog.mock.calls.some(c => String(c.arguments[0]).includes('digest tick failed')),
      'the crashed job is reported in the log',
    );
  } finally {
  ...
  const byName = (a: string) => sends.filter(s => s.automation === a);
  ...
  assert.equal((await db.select().from(automationSends)).length, sends.length,
    'the claim ledger prevents any double-send across the failed and recovered ticks');
});
