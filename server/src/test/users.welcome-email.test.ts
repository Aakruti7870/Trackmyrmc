...
// The welcome email is stubbed at the module boundary so the test never touches
// real SMTP. `emailBehavior` lets each test pick the success or failure path,
// and `welcomeCalls` records the arguments the route passed to the sender.
type EmailBehavior = 'success' | 'throw';
let emailBehavior: EmailBehavior = 'success';
let welcomeCalls: Array<{ email: string; name: string; role: string }> = [];
