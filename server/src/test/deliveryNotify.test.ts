...
// Stub the email module so notifyChallanStatus never touches real SMTP and we
// can observe exactly what it tries to send. Mock must be registered before the
// unit under test (which imports sendDeliveryNotificationEmail) is loaded.
type SentCall = { email: string; name: string; details: Record<string, unknown> };
let sent: SentCall[] = [];
let behavior: 'success' | 'throw' = 'success';
...
let challanSeq = 0;
async function seedChallan(opts: { email?: string | null; withSite?: boolean }) {
...
beforeEach(async () => {
  ...
  behavior = 'success';
  await db.execute(
    sql`TRUNCATE TABLE challans, recurring_orders, orders, sites, clients RESTART IDENTITY CASCADE`,
  );
});
