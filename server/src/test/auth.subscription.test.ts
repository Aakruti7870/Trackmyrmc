...
test('the reviewer demo login cannot bypass the billing gate', async () => {
  ...
  try {
    ...
    if (savedEmail === undefined) delete process.env.REVIEW_DEMO_EMAIL;
    else process.env.REVIEW_DEMO_EMAIL = savedEmail;
    if (savedCode === undefined) delete process.env.REVIEW_DEMO_OTP;
    else process.env.REVIEW_DEMO_OTP = savedCode;
  }
});
