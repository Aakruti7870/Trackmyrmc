...
async function createPlant(extra: Partial<typeof plants.$inferInsert> = {}) {
  seq += 1;
...
async function createClientRow(plantId: number | null = null) {
  seq += 1;
...
test('client list flags kycVerified when a linked user completed Aadhaar eKYC', async () => {
  ...
  const byId = new Map(res.body.map((c: { id: number; kycVerified: boolean }) => [c.id, c.kycVerified]));
...
test('staff order list carries customerKycVerified for the ordering client', async () => {
  ...
  assert.equal(res.status, 200);
  const order = res.body.find((o: { orderNo: string }) => o.orderNo === 'KB-ORD-1');
  assert.ok(order);
...
test('plant listings flag gstPanVerified only when the approved profile has BOTH GST and PAN', async () => {
  ...
  const dirById = new Map(dir.body.map((p: { id: number; gstPanVerified: boolean }) => [p.id, p.gstPanVerified]));
...
test('an uploaded PAN document satisfies the PAN half of the badge', async () => {
  ...
  assert.equal(res.status, 200);
  const row = res.body.find((p: { id: number }) => p.id === plant.id);
  assert.ok(row);
