...
test('platformOnly role event excludes plant-bound users of an allowed role', () => {
  ...
  emitSSEEvent(
    'whatsapp.message',
    { phone: '+919991112222', preview: 'hi' },
    { roles: ['authority', 'admin'], platformOnly: true },
  );
  ...
  assert.ok(
    !eventsOf(plantAdmin).includes('whatsapp.message'),
    'a plant-bound admin must NOT receive cross-tenant chat previews',
  );
  assert.ok(
    !eventsOf(dispatcher).includes('whatsapp.message'),
    'a role outside the allow-list never receives the event',
  );
});
...
test('plant-scoped role event reaches only staff of that plant (plus platform staff)', () => {
  ...
  emitSSEEvent(
    ...
    { orderNo: 'ORD-001', clientName: 'Acme' },
    { roles: ['admin', 'plant_owner', 'supervisor', 'dispatcher'], plantId: 5 },
  );

  assert.ok(
    eventsOf(samePlantAdmin).includes('order.pending_approval'),
    'a staff member of the issuing plant receives the approval toast',
  );
  assert.ok(
    eventsOf(samePlantDispatcher).includes('order.pending_approval'),
    'every approval role at the issuing plant is notified',
  );
  assert.ok(
    eventsOf(platformAdmin).includes('order.pending_approval'),
    'a null-plant platform admin oversees all plants and still receives it',
  );
  assert.ok(
    !eventsOf(otherPlantAdmin).includes('order.pending_approval'),
    'a staff member of ANOTHER plant must NOT receive cross-tenant order metadata',
  );
});
...
test('plant-scoped default-staff event excludes other-plant staff', () => {
  ...
  assert.ok(
    eventsOf(samePlantOperator).includes('order.updated'),
    'same-plant staff receive the plant-scoped event',
  );
  assert.ok(
    !eventsOf(otherPlantOperator).includes('order.updated'),
    'other-plant staff must NOT receive the plant-scoped event',
  );
});
