...
async function createUser(role: string, email: string, linkedClientId?: number, plantId?: number | null) {
...
async function createClient(plantId?: number | null) {
...
async function placeCustomerOrder(clientAuth: string, overrides: Record<string, unknown> = {}) {
...
beforeEach(async () => {
  await db.execute(
    sql`TRUNCATE TABLE challans, orders, clients, audit_logs, users, login_attempts, plants RESTART IDENTITY CASCADE`,
  );
  plantSeq = 0;
});
...
async function seedPendingOrderForPlant(plantId: number, orderNo: string) {
