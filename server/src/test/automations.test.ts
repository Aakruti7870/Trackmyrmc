...
test('GET lists all 8 automations with cleanup ON and everything else OFF by default', async () => {
  ...
  for (const item of res.body.items) {
    if (item.name === 'cleanup') assert.equal(item.enabled, true, 'cleanup defaults ON');
    else assert.equal(item.enabled, false, `${item.name} must default OFF`);
...
test('a plant-scoped admin writes a plant override; the global default is untouched', async () => {
  ...
  const item = globalView.body.items.find((i: { name: string }) => i.name === 'orderReminders');
...
test('POST /run works for both scopes: plant staff get a plant-scoped run, global staff a full tick', async () => {
  const plant = await createPlant('Plant Run');
  const plantAdmin = await createUser('admin', 'plant-admin@test.com', plant.id);
  const authority = await createUser('authority', 'authority@test.com', null);

  const scoped = await request(app).post('/api/automations/run')
    .set('Authorization', `Bearer ${tokenFor(plantAdmin)}`).send({});
  assert.equal(scoped.status, 200, 'a plant-bound actor may run their own plant on demand');
  assert.equal(scoped.body.scope, 'plant');

  const ok = await request(app).post('/api/automations/run')
    .set('Authorization', `Bearer ${tokenFor(authority)}`).send({});
  assert.equal(ok.status, 200);
  assert.equal(ok.body.scope, 'global');
});
...
test('claimSend grants exactly one claim per target+window, even under concurrency', async () => {
  const results = await Promise.all(
    Array.from({ length: 5 }, () => claimSend('orderReminders', 'order', 1, 'd:2026-07-03')),
  );
...
test('runOrderReminders processes tomorrow\'s orders once, then never again for the same date', async () => {
  ...
  await saveAutomationSettings('orderReminders', null, true,
    sanitizeConfig('orderReminders', { sendHour: 0, email: false, push: false, whatsapp: false }));
