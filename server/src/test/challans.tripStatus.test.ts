...
beforeEach(async () => {
  await db.execute(
    sql`TRUNCATE TABLE trip_sessions, tracking_tokens, challans, drivers, clients, audit_logs, users, login_attempts RESTART IDENTITY CASCADE`,
  );
});
...
test('the staff live-drivers board lists a checked-in driver with their open trip status', async () => {
  ...
  assert.equal(res.status, 200);
  const me = res.body.drivers.find((d: { userId: number }) => d.userId === user.id);
...
test('a GPS-inactive driver keeps their board row but their stale fix expires', async () => {
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LIVE_GPS_STALE_MS;
    else process.env.LIVE_GPS_STALE_MS = prev;
  }
});
