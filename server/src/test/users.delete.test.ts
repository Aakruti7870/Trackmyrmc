...
test('restore-all → unlink → restore: a batch surfaces the conflicting account in skippedDetails, unlinking it audits a client_link_change, and the follow-up restore then succeeds', async () => {
  ...
  const skipped = batch.body.skippedDetails.find((d: { id: number }) => d.id === conflicted.id);
...
test('restore-all: a single batch with an app-level link skip, a DB-23505 skip, and clean rows accounts for all three correctly', async (t) => {
  ...
  const byId = new Map<number, { id: number; email: string; reason: string; conflictUserId?: number; conflictUserName?: string; conflictLinkType?: string }>(
    res.body.skippedDetails.map((d: { id: number }) => [d.id, d]),
  );
