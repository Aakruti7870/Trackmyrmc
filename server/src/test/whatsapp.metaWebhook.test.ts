...
beforeEach(async () => {
  ...
  process.env.WHATSAPP_META_VERIFY_TOKEN = VERIFY_TOKEN;
  await db.execute(
    sql`TRUNCATE TABLE whatsapp_messages, challans, orders, clients, users, plants RESTART IDENTITY CASCADE`,
  );
});

after(async () => {
  if (savedVerify === undefined) delete process.env.WHATSAPP_META_VERIFY_TOKEN;
  else process.env.WHATSAPP_META_VERIFY_TOKEN = savedVerify;
  if (savedSecret === undefined) delete process.env.WHATSAPP_META_APP_SECRET;
  else process.env.WHATSAPP_META_APP_SECRET = savedSecret;
  await pool.end();
});

async function seedMessage(over: Partial<typeof whatsappMessages.$inferInsert> = {}) {
  const [row] = await db
...
function statusPayload(id: string, status: string, errorCode?: number) {
  return {
