...
async function createPlant(opts: { name?: string; subscriptionStatus?: string } = {}) {
  const [row] = await db.insert(plants).values({
    name: opts.name ?? 'Test Plant',
    plantCode: `TP${Math.floor(Math.random() * 100000)}`,
    latitude: '18.5204',
    longitude: '73.8567',
    subscriptionStatus: (opts.subscriptionStatus ?? 'active') as typeof plants.$inferInsert.subscriptionStatus,
  }).returning();
  return row;
}

async function createUser(opts: {
  email: string; role?: string; withPassword?: boolean; isActive?: boolean;
  plantId?: number | null;
}) {
  const [row] = await db.insert(users).values({
    ...
    role: (opts.role ?? 'dispatcher') as typeof users.$inferInsert.role,
...
async function setExemptRoles(csv: string) {
...
beforeEach(async () => {
  await db.execute(
    sql`TRUNCATE TABLE audit_logs, staff_otp_codes, users, plants, login_attempts, rate_limit_hits, app_settings RESTART IDENTITY CASCADE`,
  );
});
