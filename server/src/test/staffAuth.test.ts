...
async function createStaff(opts: {
  ...
  withPassword?: boolean; phone?: string | null;
}) {
...
beforeEach(async () => {
  await db.execute(
    sql`TRUNCATE TABLE audit_logs, staff_otp_codes, users, login_attempts, rate_limit_hits, app_settings RESTART IDENTITY CASCADE`,
  );
});
