...
async function insertUser(opts: {
  name: string;
  email: string;
  role?: 'admin' | 'dispatcher' | 'plant_operator' | 'client' | 'driver';
...
test('DB rejects a direct INSERT of a second live user linked to the same client (23505)', async () => {
  ...
  await assert.rejects(
    insertUser({ name: 'Second', email: 'second@test.com', linkedClientId: client.id }),
    (err: unknown) => {
...
test('DB rejects a direct INSERT of a second live user linked to the same driver (23505)', async () => {
  ...
  await assert.rejects(
    insertUser({ name: 'Second', email: 'second@test.com', role: 'driver', linkedDriverId: driver.id }),
    (err: unknown) => {
...
test('DB rejects a direct UPDATE that points a second live user at an already-linked client (23505)', async () => {
  ...
  await assert.rejects(
    db.update(users).set({ linkedClientId: client.id }).where(eq(users.id, other.id)),
    (err: unknown) => {
      assertUniqueViolation(err, 'users_linked_client_unique');
      return true;
    },
...
test('DB rejects a direct UPDATE that points a second live user at an already-linked driver (23505)', async () => {
  ...
  await assert.rejects(
    db.update(users).set({ linkedDriverId: driver.id }).where(eq(users.id, other.id)),
    (err: unknown) => {
      assertUniqueViolation(err, 'users_linked_driver_unique');
      return true;
    },
...
test('partial index allows re-linking a client once the holding account is soft-deleted', async () => {
  ...
  await assert.rejects(
    insertUser({ name: 'Blocked', email: 'blocked@test.com', linkedClientId: client.id }),
    (err: unknown) => { assertUniqueViolation(err, 'users_linked_client_unique'); return true; },
  );
  ...
  await assert.rejects(
    insertUser({ name: 'Blocked2', email: 'blocked2@test.com', linkedClientId: client.id }),
    (err: unknown) => { assertUniqueViolation(err, 'users_linked_client_unique'); return true; },
  );
});
...
test('partial index allows re-linking a driver once the holding account is soft-deleted', async () => {
  ...
  await assert.rejects(
    insertUser({ name: 'Blocked', email: 'blocked@test.com', role: 'driver', linkedDriverId: driver.id }),
    (err: unknown) => { assertUniqueViolation(err, 'users_linked_driver_unique'); return true; },
  );
  ...
  await assert.rejects(
    insertUser({ name: 'Blocked2', email: 'blocked2@test.com', role: 'driver', linkedDriverId: driver.id }),
    (err: unknown) => { assertUniqueViolation(err, 'users_linked_driver_unique'); return true; },
  );
});
