...
import {
  users, clients, orders, challans, plants, drivers,
  aiSettings, aiConversations, aiMessages, supportTickets,
} from '../db/schema.js';
...
async function createUser(
  ...
  opts: { plantId?: number | null; linkedClientId?: number | null; linkedDriverId?: number | null; isActive?: boolean } = {},
) {
...
async function createClient(plantId: number, code: string, overrides: Partial<typeof clients.$inferInsert> = {}) {
...
async function createOrder(plantId: number, clientId: number, orderNo: string, grade = 'M25') {
...
async function createChallan(plantId: number, clientId: number, challanNo: string, opts: { driverId?: number; grade?: string } = {}) {
...
function tokenFor(u: { id: number; email: string; role: string; name: string; plantId?: number | null; linkedClientId?: number | null; linkedDriverId?: number | null }) {
  return signToken({
...
function chat(token: string | null, body: Record<string, unknown>) {
