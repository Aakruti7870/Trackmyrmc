...
beforeEach(async () => {
  ...
  await db.execute(
    sql`TRUNCATE TABLE whatsapp_messages, clients, users, plants RESTART IDENTITY CASCADE`,
  );
});
...
async function seedInbound(over: Partial<typeof whatsappMessages.$inferInsert> = {}) {
...
function inboundPayload(wamid: string, from: string, text: string, profileName = 'Ramesh') {
  return {
...
test('GET /chats lists conversations for a global admin with window state', async () => {
  ...
  const mine = res.body.find((c: { phone: string }) => c.phone === CUSTOMER);
  assert.ok(mine);
