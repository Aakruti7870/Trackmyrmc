...
async function alertStaleStaff(
  stale: { userId: number; name: string | null; role: string | null; plantId: number | null }[],
): Promise<void> {
  try {
    for (const s of stale) {
      ...
      emitSSEEvent(
        'duty.stale',
        { userId: s.userId, name, role: s.role, plantId: s.plantId },
        staleAlertAudience(s.plantId),
      );
