...
export type LivePosition = {
  ...
  // Owning plant of the underlying challan; used to tenant-scope the staff
  // live map (a plant-bound staffer must only see their own plant's trucks).
  plantId: number | null;
  ...
  siteReleaseTime: string | null;
  updatedAt: string;
  // Stationary-anchor + alert dedupe state for theft detection. Kept in-memory
  // alongside the live fix; the alerts themselves are persisted when raised.
...
let fuelCfgCache: { cfg: FuelConfig; at: number } | null = null;
const FUEL_CFG_TTL_MS = 60_000;
async function getFuelConfigCached(): Promise<FuelConfig> {
  const now = Date.now();
  if (fuelCfgCache && now - fuelCfgCache.at < FUEL_CFG_TTL_MS) return fuelCfgCache.cfg;
...
async function persistVehicleAlert(a: {
  ...
  vehicleId: number | null;
  driverId: number | null;
  type: 'unscheduled_stop' | 'route_deviation';
  lat: number;
  lng: number;
  distanceM: number;
  detail: string;
...
router.post('/', requireRole('driver'), async (req, res) => {
  ...
  const withinRadius =
    distanceM != null && distanceM <= GEOFENCE_RADIUS_M && (acc == null || acc <= MAX_ACCURACY_M);
  ...
  let arrivalTime: Date | null = row.siteArrivalTime;
  let releaseTime: Date | null = row.siteReleaseTime;
  ...
  if (arrivalTime != null && releaseTime == null) {
    const outsideRelease =
      distanceM != null && distanceM > RELEASE_RADIUS_M && (acc == null || acc <= MAX_ACCURACY_M);
  ...
  if (goodAccuracy && hasPlant) {
    ...
    if (status === 'dispatched' && row.siteLat != null && row.siteLng != null) {
      const offCorridorM = pointToSegmentM(
        latitude, longitude,
        plantLat, plantLng,
        Number(row.siteLat), Number(row.siteLng),
      );
  ...
  if (releaseTime != null) livePositions.delete(cid);
...
router.get('/alerts', requireRole('admin', 'dispatcher', 'authority'), async (req, res) => {
  ...
  const rows = await db
    ...
    .where(and(
      onlyOpen ? isNull(vehicleAlerts.acknowledgedAt) : undefined,
      plantScope(req.user!.plantId, challans.plantId),
    ))
...
export interface FreshnessLoad extends FreshnessResult {
  challanId: number;
  challanNo: string;
  clientId: number | null;
  ...
  /** Whether a live GPS fix has been received for this load yet. */
...
export async function getFreshnessLoads(now = new Date(), actorPlantId: number | null = null): Promise<FreshnessResponse> {
  ...
  const loads: FreshnessLoad[] = rows.map(row => {
  ...
  const rank: Record<string, number> = { expired: 0, critical: 1, warning: 2, fresh: 3 };
  loads.sort((a, b) => {
