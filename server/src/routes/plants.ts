...
import {
  discoverConcretePlants,
  discoverSuppliers,
  isDiscoveryConfigured,
  isSupplierCategoryKey,
  SUPPLIER_CATEGORIES,
...
router.get('/discover-suppliers', requireRole('authority'), discoverSuppliersLimiter, async (req, res) => {
  ...
  const categories: SupplierCategoryKey[] = requested.length
    ? requested
    : SUPPLIER_CATEGORIES.map(c => c.key);
  ...
  try {
    ...
    const out = suppliers
      ...
      .filter(
        s =>
          !onboarded.some(
            op => haversineKm(s.latitude, s.longitude, parseFloat(op.latitude), parseFloat(op.longitude)) < 0.15,
          ),
      )
      .sort((a, b) => a.distanceKm - b.distanceKm);
    res.json(out);
...
router.post('/invite', async (req, res) => {
  ...
  if (!deduped) void notifyNewPlantRequest(row);
});
...
async function notifyNewPlantRequest(row: typeof plantInvites.$inferSelect): Promise<void> {
  ...
  try {
    ...
    for (const e of notifyCfg.recipients) emails.add(e);
      ...
      const staff = await db
        ...
        .where(
          and(
            ...
            inArray(users.role, notifyCfg.roles),
          ),
        );
...
const NETWORK_STATUSES = ['pending', 'invited', 'verified', 'active'] as const;
...
function parseBody(body: Record<string, unknown>) {
  ...
  if (body.ownerName !== undefined) out.ownerName = optStr(body.ownerName);
  if (body.whatsappNumber !== undefined) out.whatsappNumber = optStr(body.whatsappNumber);
  if (body.showOnNetwork !== undefined) out.showOnNetwork = Boolean(body.showOnNetwork);
  if (body.networkStatus !== undefined && NETWORK_STATUSES.includes(body.networkStatus as NetworkStatus)) {
...
router.post('/', ADMIN, platformStaffOnly, async (req, res) => {
  ...
  if (data.verified === true) {
    ...
    if (data.networkStatus === undefined) data.networkStatus = 'active';
  }
...
function missingVerifyFields(src: {
  legalName?: unknown; gstNo?: unknown; contactNumber?: unknown; email?: unknown;
}): string[] {
