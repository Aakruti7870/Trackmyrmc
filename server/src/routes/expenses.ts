...
const REVIEW_ROLES = ['authority', 'admin', 'plant_owner', 'supervisor', 'fleet_manager', 'accountant'] as const;
...
function parseExpenseAt(value: unknown): Date {
  if (value === undefined || value === null || value === '') return new Date();
  const d = new Date(value as string);
  if (Number.isNaN(d.getTime())) throw new Error('Expense date/time must be valid');
  return d;
}
...
function parseCoord(value: unknown, kind: 'lat' | 'lng'): string | null | undefined {
...
function validateReceipt(value: unknown): string | null | undefined {
  if (value === undefined) return undefined;
  if (value === null || value === '') return null;
  if (typeof value !== 'string') throw new Error('Receipt must be a string');
  if (!isObjectStoragePath(value)) throw new Error('Receipt must be an uploaded object path');
  if (value.length > MAX_OBJECT_PATH_CHARS) throw new Error('Receipt path is too long');
  return value;
}
...
async function actingDriver(userId: number): Promise<{ driverId: number | null; plantId: number | null }> {
  const [row] = await db
...
router.get('/:id', async (req, res) => {
  ...
  const isReviewer = (REVIEW_ROLES as readonly string[]).includes(actor.role)
    && (actor.plantId == null || actor.plantId === row.plantId);
  if (!isOwner && !isReviewer) { res.status(404).json({ error: 'Not found' }); return; }
...
router.post('/', requireRole('driver'), async (req, res) => {
  let category: string, amount: string, expenseAt: Date;
  let gpsLat: string | null | undefined, gpsLng: string | null | undefined;
...
router.delete('/:id', requireRole('driver'), async (req, res) => {
  ...
  if (existing.status !== 'pending') { res.status(409).json({ error: 'Only a pending expense can be deleted' }); return; }
