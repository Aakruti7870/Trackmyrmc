...
router.get('/config', async (req, res) => {
  ...
  let plantOptions: { id: number; name: string; plantCode: string | null }[] | undefined;
...
async function getOrCreateConversation(userId: number, sessionId: string, role: string, scopePlantId: number | null) {
...
async function prepareChat(user: AuthPayload, body: Record<string, unknown>): Promise<PreparedChat> {
  ...
  const history = prior.reverse().flatMap(p => {
    const turns: { role: 'user' | 'assistant'; content: string }[] = [{ role: 'user', content: p.message }];
    if (p.response) turns.push({ role: 'assistant', content: p.response });
    return turns;
  });
...
async function persistMessage(
  conversationId: number, sessionId: string, userId: number, plantId: number | null,
  message: string, response: string, inputType: string, outputType: string, functionsUsed: string,
) {
  try {
...
router.post('/support-ticket', rateLimit({ windowMs: 60_000, max: 5, name: 'ai_support_ticket' }), async (req, res) => {
  ...
  const plantId: number | null = scopePlantId ?? user.plantId ?? null;
  const clientId: number | null = user.linkedClientId ?? null;
