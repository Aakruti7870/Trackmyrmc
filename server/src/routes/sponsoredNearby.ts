...
const packageWeight: Record<string, number> = { premium: 300, featured: 200, standard: 100, custom: 150 };
...
router.get('/nearby', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [profiles, promotions, gstPanVerified] = await Promise.all([
      ...
      ids.length ? db.select().from(plantPromotions).where(and(inArray(plantPromotions.plantId, ids), eq(plantPromotions.isActive, true), inArray(plantPromotions.paymentStatus, ['paid', 'waived']))) : [],
      getGstPanVerifiedPlantIds(),
    ]);
    const profileByPlant = new Map(profiles.map(p => [p.plantId, p]));
    const promoByPlant = new Map(promotions.filter(p => p.startAt <= new Date() && p.endAt >= new Date()).map(p => [p.plantId, p]));
    const out = visiblePlants.map(p => {
      ...
      const promotion = promoByPlant.get(p.id);
      const sponsored = Boolean(promotion && distanceKm <= Number(promotion.promotionRadiusKm));
      return {
        ...
        bannerTitle: sponsored ? promotion!.bannerTitle : null,
        bannerSubtitle: sponsored ? promotion!.bannerSubtitle : null,
        showGlowingEffect: sponsored ? promotion!.showGlowingEffect : false,
        _rank: sponsored ? promotion!.priorityRank : Number.MAX_SAFE_INTEGER,
        _weight: sponsored ? (packageWeight[promotion!.packageType] ?? 0) : 0,
        _updated: sponsored ? promotion!.updatedAt.getTime() : 0,
      };
    }).filter(Boolean) as any[];
    out.sort((a, b) => {
      if (a.isSponsored !== b.isSponsored) return a.isSponsored ? -1 : 1;
      if (a.isSponsored) return a._rank - b._rank || b._weight - a._weight || b._updated - a._updated || a.distanceKm - b.distanceKm || a.name.localeCompare(b.name);
      return a.distanceKm - b.distanceKm || a.name.localeCompare(b.name);
    });
    res.status(200).json({ plants: out.map(({ _rank, _weight, _updated, ...p }) => p), count: out.length });
  } catch (error) {
    console.error('[plants] sponsored nearby lookup failed:', error);
