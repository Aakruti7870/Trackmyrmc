...
router.post('/', requireRole(...WRITE_ROLES), async (req, res) => {
  ...
  if (orderId) {
    ...
    if (o.status && BLOCKED.includes(o.status)) {
      res.status(409).json({ error: 'This order is not approved for dispatch yet.' }); return;
    }
    if (o.plantId != null) plantId = o.plantId;
  }
  ...
  if (trip) emitSSEEvent('trip.updated', trip, { clientId: trip.clientId, driverId: trip.driverId, plantId: trip.plantId });
  ...
  if (process.env.NODE_ENV === 'test') await tripShareTask;
  else void tripShareTask;
...
router.put('/:id', async (req, res) => {
  ...
  if (role === 'driver') {
    ...
    if (closed) emitSSEEvent('trip.updated', closed, { clientId: closed.clientId, driverId: closed.driverId, plantId: closed.plantId });
...
router.post('/:id/trip-status', requireRole('driver'), async (req, res) => {
  ...
  if (!(DRIVER_TRIP_STATUSES as readonly string[]).includes(status)) {
  ...
  const result = await advanceTripStatus(challanId, status as (typeof DRIVER_TRIP_STATUSES)[number]);
  if (!result.ok) {
...
router.post('/:id/share', requireRole(...WRITE_ROLES), async (req, res) => {
  ...
  if (!Number.isInteger(id)) { res.status(400).json({ error: 'Invalid challan id' }); return; }
