...
import { plants, clients, orders, challans, vehicles, fuelLogs, automationSends, } from '../db/schema.js';
import { istParts, loadAutomationSnapshot, periodKey, sanitizeConfig, saveAutomationSettings, } from '../lib/automations.js';
import { runDigests, runDeliveryFollowUps, runFuelAnomalyAlerts, runWinBacks, runIdleVehicleAlerts, } from '../lib/automationJobs.js';
...
async function createClient(name, plantId) {
    const [c] = await db.insert(clients).values({
        name, contactPerson: 'Contact', phone: '9990001111', plantId,
    }).returning();
    return c;
}
async function createVehicle(vehicleNo, plantId, extra = {}) {
    const [v] = await db.insert(vehicles).values({
        vehicleNo, capacity: '8', plantId, ...extra,
    }).returning();
    return v;
}
async function enable(name, config, plantId = null, enabled = true) {
