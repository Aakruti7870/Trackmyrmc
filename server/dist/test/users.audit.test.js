...
test('GET /api/audit-logs?action filters by event type', async () => {
    ...
    assert.ok(res.body.rows.every((e) => e.action === 'password_reset'), 'every returned entry has the filtered action');
});
test('GET /api/audit-logs?from/to filters by date range (to inclusive of whole day)', async () => {
    ...
    const res = await request(app)
        ...
        .set('Authorization', `Bearer ${tokenFor(admin)}`);
    assert.equal(res.status, 200);
    const emails = res.body.rows.map((e) => e.targetUserEmail).sort();
...
test('GET /api/audit-logs?actorId filters to one actor', async () => {
    ...
    assert.ok(res.body.rows.every((e) => e.actorId === admin.id), 'every returned entry was performed by the filtered actor');
});
...
test('GET /api/audit-logs paging: limit/offset slice and hasMore/total report accurately', async () => {
    ...
    assert.deepEqual(page1.body.rows.map((e) => e.targetUserEmail), ['e5@x.com', 'e4@x.com'], 'first page returns the two newest rows');
    ...
    assert.deepEqual(page2.body.rows.map((e) => e.targetUserEmail), ['e3@x.com', 'e2@x.com'], 'second page continues after the first page with no overlap');
    ...
    assert.deepEqual(page3.body.rows.map((e) => e.targetUserEmail), ['e1@x.com'], 'the final page returns the remaining row');
...
test('GET /api/audit-logs/facets lists the distinct actions', async () => {
    ...
    assert.deepEqual([...res.body.actions].sort(), ['password_reset', 'user.created'], 'each distinct action appears exactly once');
});
