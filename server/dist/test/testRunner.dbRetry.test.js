...
// spawning a database.
import { isTransientDbError, withAdminRetry, TRANSIENT_DB_CODES, } from '../../scripts/dbAdminRetry.mjs';
...
test('withAdminRetry: returns the result without sleeping when fn succeeds first try', async () => {
    ...
    let calls = 0;
    const result = await withAdminRetry('op', () => {
        calls += 1;
        return 'ok';
    }, { sleep, log: noopLog });
...
test('withAdminRetry: retries transient errors and eventually succeeds', async () => {
    ...
    const result = await withAdminRetry('op', () => {
        calls += 1;
        if (calls < 3)
            throw dbErr('57P03'); // cannot_connect_now (twice)
        return 'recovered';
    }, { sleep, log: noopLog });
...
test('withAdminRetry: gives up after the configured number of tries', async () => {
    const { waits, sleep } = fakeSleep();
    let calls = 0;
    await assert.rejects(withAdminRetry('op', () => {
        calls += 1;
        throw dbErr('53300'); // too_many_connections — always transient
    }, { tries: 4, delayMs: 10, sleep, log: noopLog }), /boom 53300/);
...
test('withAdminRetry: rethrows a permanent error immediately without retrying', async () => {
    ...
    }, { tries: 8, sleep, log: noopLog }), /boom 42P04/);
...
test('withAdminRetry: passes the attempt number to fn', async () => {
    ...
    const seen = [];
    await withAdminRetry('op', (attempt) => {
        seen.push(attempt);
        if (attempt < 3)
            throw dbErr('40001'); // serialization_failure
        return null;
    }, { sleep, log: noopLog });
