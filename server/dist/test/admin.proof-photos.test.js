...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challan_proof_photos, challans, clients, audit_logs, users RESTART IDENTITY CASCADE`);
});
after(async () => {
    await pool.end();
});
...
test('reports only the rows still stored as base64 as stuck', async () => {
    ...
    assert.deepEqual(res.body.photos.map((p) => p.id).sort((a, b) => a - b), [stuckA.id, stuckB.id].sort((a, b) => a - b));
...
test('partial failure: still-stuck rows come back as a fresh actionable report and audit-logs failure', async (t) => {
    ...
    const storeMock = t.mock.method(proofPhotoStore, 'store', async (dataUrl) => {
        if (dataUrl === BASE64_PHOTO)
            return '/objects/uploads/recovered';
