...
const ENV_KEYS = ['APP_URL', 'PUBLIC_URL', 'REPLIT_DEV_DOMAIN'];
const savedEnv = {};
let app;
let token = '';
let clientId = 0;
...
async function enableTripShare() {
    await saveAutomationSettings('tripShare', null, true, sanitizeConfig('tripShare', { ttlHours: 24, email: true, push: false, whatsapp: false }));
}
...
async function waitForEmails(expected, timeoutMs = 5000) {
    const deadline = Date.now() + timeoutMs;
...
test('a dispatched challan with tripShare on records exactly one send with a working share link', async () => {
    ...
    assert.equal(challan.tripShareWarning, undefined, 'a reachable customer (email on file) produces no dispatch warning');
    ...
    assert.match(url, /^https:\/\/track\.example\.com\/track\/[0-9a-f]{48}$/, 'the share link points at the configured base URL');
...
test('dispatching to a customer with no email surfaces tripShareWarning and no email goes out', async () => {
    ...
    const challan = await dispatchChallan();
    assert.equal(challan.tripShareWarning, 'no_email', 'the dispatch response tells staff the tracking link was not sent');
...
test('no warning when tripShare is off, and a WhatsApp fallback suppresses it', async () => {
    ...
    await saveAutomationSettings('tripShare', null, true, sanitizeConfig('tripShare', { ttlHours: 24, email: true, push: false, whatsapp: true, whatsappTemplate: 'trip_tpl' }));
    const viaWhatsApp = await dispatchChallan();
    assert.equal(viaWhatsApp.tripShareWarning, undefined, 'a WhatsApp-reachable customer produces no warning');
});
