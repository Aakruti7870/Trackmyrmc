...
async function createClientRow(name) {
    const [row] = await db.insert(clients).values({
        name, contactPerson: 'Jane', phone: '1112223333',
    }).returning();
    return row;
}
async function createDriverRow(name) {
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challans, orders, drivers, clients, audit_logs, users, login_attempts RESTART IDENTITY CASCADE`);
});
const open = [];
afterEach(() => {
    for (const c of open)
...
test('a client connection is tagged with the user\'s linked client id', async () => {
    ...
    assert.ok(conn.events().includes('match.event'), 'the handler must derive the linked client id so own-company events arrive');
    assert.ok(!conn.events().includes('wrong.event'), "another company's events must NOT reach this client");
});
test('a driver connection is tagged with the user\'s linked driver id', async () => {
    ...
    const sawSentinel = await waitFor(() => conn.events().includes('sentinel'));
    assert.ok(sawSentinel, 'sentinel broadcast should reach the connection');
    assert.ok(conn.events().includes('match.event'), 'the handler must derive the linked driver id so own-trip events arrive');
    assert.ok(!conn.events().includes('wrong.event'), "another driver's events must NOT reach this driver");
});
test('a client connection does not receive a different company by driver targeting', async () => {
    ...
    const sawSentinel = await waitFor(() => conn.events().includes('sentinel'));
    assert.ok(sawSentinel, 'sentinel broadcast should reach the connection');
    assert.ok(!conn.events().includes('driver.only'), 'a client must not receive driver-targeted events');
});
test('closing a connection unregisters it so it no longer receives events', async () => {
    ...
    assert.ok(unregistered, 'closing the socket must call removeSSEClient so the count drops back');
    ...
    assert.equal(conn.events().length, eventsBeforeClose, 'no events should be written to the stream after it was closed');
    assert.ok(!conn.events().includes('after.close'), 'events emitted after close must not reach the unregistered connection');
});
