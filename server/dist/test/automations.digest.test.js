...
mock.module('../lib/whatsappRetry.js', {
    namedExports: {
        ...
        sendWhatsAppWithRetry: async (phone, template, vars) => {
...
async function enableDigest(config) {
    await saveAutomationSettings('digest', null, true, sanitizeConfig('digest', { frequency: 'daily', sendHour: 0, ...config }));
}
before(() => {
