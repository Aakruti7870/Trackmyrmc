...
test('filters by action type', async () => {
    ...
    assert.ok(res.body.rows.every((r) => r.action === 'password_reset'));
});
...
test('facets returns distinct actions and actors', async () => {
    ...
    assert.ok(res.body.actors.every((a) => a.deleted === false));
});
test('facets surfaces deleted actors (null id, preserved name)', async () => {
    ...
    assert.equal(res.status, 200);
    assert.equal(res.body.actors.length, 2);
    const live = res.body.actors.find((a) => !a.deleted);
    const deleted = res.body.actors.find((a) => a.deleted);
...
test('filters by a deleted actor name (null-actor rows only)', async () => {
    ...
    assert.ok(res.body.rows.every((r) => r.actorId === null && r.actorName === 'Departed Admin'));
});
...
test('pages through results with limit + offset, exposing hasMore', async () => {
    ...
    await seedAudit(Array.from({ length: 5 }, (_, i) => ({
        action: 'user.created',
...
test('paging respects active filters', async () => {
    ...
    assert.equal(page1.body.rows.length, 2);
    assert.equal(page1.body.hasMore, true);
    assert.ok(page1.body.rows.every((r) => r.action === 'password_reset'));
    const page2 = await request(app)
