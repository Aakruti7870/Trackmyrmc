...
test('restore-all: a row that trips the DB link constraint (23505) is skipped, the rest still restore', async (t) => {
    ...
    assert.match(res.body.skippedDetails[0].reason, /already linked/i, 'the skip carries the same friendly link-conflict reason');
...
const NON_ADMIN_ROLES = ['dispatcher', 'plant_operator', 'client', 'driver'];
