...
async function createClientUser(email, clientId) {
...
async function createDriverUser(email) {
    const passwordHash = await bcrypt.hash(PASSWORD, 10);
...
let seq = 0;
async function createChallan(clientId, driverId) {
    seq += 1;
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challans, drivers, clients, users RESTART IDENTITY CASCADE`);
});
