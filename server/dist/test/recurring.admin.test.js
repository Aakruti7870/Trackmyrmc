...
async function createTemplate(clientId, opts = {}) {
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE recurring_orders, orders, sites, clients, users RESTART IDENTITY CASCADE`);
});
...
test('admin lists recurring templates across all clients with names', async () => {
    ...
    const names = res.body.map((r) => r.clientName).sort();
    assert.deepEqual(names, ['Alpha', 'Beta']);
    const withSite = res.body.find((r) => r.siteName === 'Site A');
    assert.ok(withSite);
});
