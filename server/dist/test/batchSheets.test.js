...
import { computeBatchSizes, computeTotals, generateActuals } from '../lib/batchSheet.js';
let app;
const MATERIALS = [
...
async function createUser(role, plantId = null, email = `${role}@test.com`) {
    const [row] = await db.insert(users).values({
        name: role, email, passwordHash: null, role, isActive: true, plantId,
    }).returning();
    return row;
}
async function createPlant(name, code) {
...
async function createMix(token, overrides = {}) {
...
test('generateActuals stays inside the material tolerance band', () => {
    const mats = MATERIALS.map((m, i) => ({ ...m, key: `k${i}` }));
    for (let run = 0; run < 50; run++) {
        const actuals = generateActuals(mats, 0.5);
        for (const m of mats) {
            const target = m.perCumKg * 0.5;
            const tol = target * (m.tolerancePct / 100);
            assert.ok(actuals[m.key] >= target - tol - 0.06 && actuals[m.key] <= target + tol + 0.06, `${m.label}: ${actuals[m.key]} outside ${target}±${tol}`);
        }
    }
});
...
test('mix design create/list/update/delete with grade uniqueness per plant', async () => {
    ...
    assert.equal(created.body.materials.length, 7);
...
test('generates a batch report: row count, targets, tolerance, totals', async () => {
    ...
    assert.equal(half.body.batches.length, 12);
...
test('editing actuals validates rows; regenerate re-rolls inside tolerance; scoping enforced', async () => {
    ...
    const batches = created.body.batches;
    // Edit one actual value.
    const mats = created.body.materials;
    ...
    assert.equal(upd.body.batches[0].actuals[key], 999);
    ...
    assert.notEqual(regen.body.batches[0].actuals[key], 999, 'regenerated');
