...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE plant_customers, challans, orders, clients, audit_logs, users, login_attempts, fuel_logs, batch_records, vehicles, plants RESTART IDENTITY CASCADE`);
    plantSeq = 0;
});
after(async () => {
    await pool.end();
...
test('fuel reconciliation is plant-scoped: a plant only sees its own vehicles\' KM', async () => {
    ...
    const seedTrip = async (plantId, plantCode, vehicleId, km) => {
        const [client] = await db.insert(clients).values({
            plantId, customerCode: `${plantCode}-C0001`, name: `${plantCode}-C0001`,
            contactPerson: 'C', phone: '9990001111',
        }).returning();
    ...
    const kmFor = (body, vid) => Number(body.rows.find(r => r.vehicleId === vid)?.km ?? 0);
    const hasVehicle = (body, vid) => body.rows.some(r => r.vehicleId === vid);
...
async function createBatch(plantId, batchNo, qty) {
