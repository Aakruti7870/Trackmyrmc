...
async function createSite(clientId, name = 'Tower A') {
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challans, sites, drivers, vehicles, clients, audit_logs, users, login_attempts RESTART IDENTITY CASCADE`);
});
after(async () => {
    await pool.end();
});
...
for (const field of ['vehicleId', 'driverId']) {
...
test('siteId is never touched by a challan edit (omitted or sent falsy)', async () => {
    ...
    assert.equal((await readChallan(challan.id)).siteId, site.id, 'an explicit falsy siteId is ignored — the saved site is not wiped');
});
...
test('omitting deliveredQuantity keeps it; an explicit null clears it', async () => {
    ...
    const omitted = await request(app)
        ...
        .send({ status: 'delivered' });
    assert.equal(omitted.status, 200);
    assert.equal((await readChallan(challan.id)).deliveredQuantity, '5.00', 'omitting deliveredQuantity leaves the recorded figure intact');
    const cleared = await request(app)
        .put(`/api/challans/${challan.id}`)
        .set('Authorization', `Bearer ${tokenFor(actor)}`)
        .send({ deliveredQuantity: null });
    assert.equal(cleared.status, 200);
    assert.equal((await readChallan(challan.id)).deliveredQuantity, null, 'an explicit null deliveredQuantity clears it on purpose');
});
