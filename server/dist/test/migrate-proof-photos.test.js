...
const BASE64_PHOTO = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M8AAAMBAQDJ/pLvAAAAAElFTkSuQmCC';
...
async function addPhoto(challanId, photo) {
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challan_proof_photos, challans, clients RESTART IDENTITY CASCADE`);
});
after(async () => {
    await pool.end();
});
...
test('already-migrated /objects/ rows are left untouched and not re-uploaded', async (t) => {
    ...
    const result = await migrateProofPhotos(db);
    assert.deepEqual(result, { migrated: 1, skipped: 0, failed: 0, failures: [] }, 'only the base64 row is migrated; the object-path row is never selected');
...
test('partial failure: failed uploads are reported and the rows stay base64 in the database', async (t) => {
    ...
    const storeMock = t.mock.method(proofPhotoStore, 'store', async () => {
        call += 1;
        if (call === 1)
            return '/objects/uploads/migrated-ok';
...
test('retry: some failed rows now succeed and the rest are written to a fresh report', async (t) => {
    ...
    assert.deepEqual([...failedIds].sort((x, y) => x - y), [rowA.id, rowB.id].sort((x, y) => x - y), 'the report lists exactly the failed row ids');
    // Retry: storage is back for rowA but rowB still fails.
    const retryMock = t.mock.method(proofPhotoStore, 'store', async (dataUrl) => {
        if (dataUrl === BASE64_PHOTO)
            return '/objects/uploads/retried-ok';
...
test('writeFailureReport writes nothing when there are no failures', async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), 'proof-photo-report-'));
    const reportPath = await writeFailureReport({ migrated: 3, skipped: 1, failed: 0, failures: [] }, dir);
