...
import { addSSEClient, removeSSEClient, getSSEClientCount, emitSSEEvent, KEEPALIVE_MS, SWEEP_MS, STALE_THRESHOLD_MS, } from '../lib/sseEmitter.js';
...
function addWithIdentity(res, identity) {
    return addSSEClient(res, identity);
}
...
test('emit: a broken connection is dropped during a broadcast while the healthy one survives', () => {
    ...
    assert.equal(getSSEClientCount(), 1, 'the broken connection must be dropped during emit, not left for the sweep');
    assert.ok(broken.ended, 'the dropped connection should have been ended');
    assert.ok(eventsOf(healthy).includes('system.notice'), 'the healthy connection must still receive the broadcast event');
...
test('emit: a connection whose write() throws synchronously is dropped mid-broadcast', () => {
    ...
    emitSSEEvent('system.notice', { message: 'hello' });
    assert.equal(getSSEClientCount(), 1, 'a connection whose write throws must be dropped during emit');
    assert.ok(eventsOf(healthy).includes('system.notice'), 'the healthy connection must still receive the broadcast event');
    created.length = 0;
    created.push(healthyId);
});
...
test('targeted event: a client only receives events for their own company', () => {
    ...
    assert.ok(!eventsOf(otherClient).includes('order.updated'), "another company's client must NOT receive the toast");
});
test('targeted event: a driver only receives events for trips assigned to them', () => {
    ...
    assert.ok(!eventsOf(otherDriver).includes('challan.updated'), 'an unassigned driver must NOT receive the toast');
});
test('targeted event: staff roles receive all order/trip toasts', () => {
    ...
    for (const [name, res] of [['admin', admin], ['dispatcher', dispatcher], ['operator', operator]]) {
...
test('live position: a client only receives positions for their own deliveries', () => {
    ...
    assert.ok(eventsOf(ownClient).includes('vehicle.position'), 'the owning client sees their truck position');
    assert.ok(!eventsOf(otherClient).includes('vehicle.position'), "another company's client must NOT see the live truck position");
});
test('live position: a driver only receives positions for trips assigned to them', () => {
    ...
    created.push(addWithIdentity(otherDriver, { role: 'driver', driverId: 5 }));
    emitSSEEvent('vehicle.position', { challanId: 1, lat: 1, lng: 2 }, { clientId: 7, driverId: 3 });
    assert.ok(eventsOf(ownDriver).includes('vehicle.position'), 'the assigned driver sees their own truck position');
    assert.ok(!eventsOf(otherDriver).includes('vehicle.position'), 'an unassigned driver must NOT see the live truck position');
});
test('live position: staff still receive every truck position for the control-room map', () => {
    ...
    emitSSEEvent('vehicle.position', { challanId: 1, lat: 1, lng: 2 }, { clientId: 7, driverId: 3 });
    for (const [name, res] of [['admin', admin], ['dispatcher', dispatcher], ['operator', operator]]) {
        assert.ok(eventsOf(res).includes('vehicle.position'), `${name} must receive every truck position for the live map`);
    }
});
