...
const KYC_ENV = ['KYC_ENABLED', 'KYC_BASE_URL', 'KYC_API_KEY', 'KYC_API_SECRET', 'KYC_API_VERSION', 'KYC_PROVIDER'];
const savedEnv = {};
...
test('kyc-settings is platform-staff only — a plant-scoped admin is blocked', async () => {
    ...
    assert.equal((await request(app).post('/api/admin/kyc-settings').set('Authorization', auth).send({ enabled: false })).status, 403);
});
