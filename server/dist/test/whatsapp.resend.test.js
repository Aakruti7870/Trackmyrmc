...
async function seedOrderMessage(opts = {}) {
    const plantId = opts.plantId ?? null;
    const client = await seedClient({ phone: opts.phone, plantId });
    seq += 1;
...
beforeEach(async () => {
    seq = 0;
    await db.execute(sql `TRUNCATE TABLE whatsapp_messages, challans, orders, clients, users, plants, app_settings RESTART IDENTITY CASCADE`);
