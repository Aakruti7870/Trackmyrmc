...
import { plants, clients, sites, drivers, vehicles, orders, challans, batchRecords, ledgerEntries, users, plantCustomers, } from '../db/schema.js';
...
const seedScript = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'db', 'seed.ts');
...
async function snapshotCounts() {
    const entries = await Promise.all(Object.keys(countTables).map(async (key) => {
        const [{ n }] = await db
...
test('re-running the seed creates no duplicate demo records', () => {
    assert.deepEqual(secondCounts, firstCounts, 'row counts must be identical after the second seed (no duplicates inserted)');
});
