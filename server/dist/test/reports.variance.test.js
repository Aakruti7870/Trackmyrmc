...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challans, clients, audit_logs, users, login_attempts RESTART IDENTITY CASCADE`);
});
after(async () => {
    await pool.end();
});
...
test('client-wise variance counts only challans with a recorded delivered qty', async () => {
    ...
    const byName = Object.fromEntries(res.body.map((r) => [r.clientName, r]));
...
test('client-wise variance is 0 when no challan has a delivered qty yet', async () => {
    ...
    const res = await request(app)
        ...
        .set('Authorization', `Bearer ${tokenFor(admin)}`);
    assert.equal(res.status, 200);
    const row = res.body.find((r) => r.clientName === 'Acme Co');
...
test('grade-wise variance aggregates the delivered subset per grade', async () => {
    ...
    const res = await request(app)
        ...
        .set('Authorization', `Bearer ${tokenFor(admin)}`);
    assert.equal(res.status, 200);
    const byGrade = Object.fromEntries(res.body.map((r) => [r.grade, r]));
