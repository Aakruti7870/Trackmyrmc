...
async function createStaffUser(name, email, role, plantId) {
    const passwordHash = await hashPassword(PASSWORD);
    const [user] = await db.insert(users).values({
...
test('plant-bound staff only see their own plant on the live map (REST + SSE); platform staff see all', async () => {
    ...
    for (const [u, ch] of [[driverAUser, challanA], [driverBUser, challanB]]) {
        const res = await request(app)
            .post('/api/positions')
    ...
    assert.equal(plantAAdminSse.events().filter(e => e === 'vehicle.position').length, 1, "a plant-bound admin must only receive their own plant's truck positions");
    assert.equal(platformAdminSse.events().filter(e => e === 'vehicle.position').length, 2, 'a platform admin must receive every truck position');
    // REST: GET /api/positions applies the same tenant scoping.
    const scoped = await request(app)
        .get('/api/positions')
        .set('Authorization', `Bearer ${tokenFor(plantAAdmin)}`);
    assert.equal(scoped.status, 200);
    assert.deepEqual(scoped.body.map((p) => p.challanId), [challanA.id], "the live-map list for a plant-bound admin must only contain their own plant's trucks");
    ...
    assert.deepEqual(alertsScoped.body.map((a) => a.id), [alertA.id], "the alert feed for a plant-bound admin must only contain their own plant's alerts");
    ...
    assert.deepEqual(freshScoped.body.loads.map((l) => l.challanId), [challanA.id], "the freshness board for a plant-bound admin must only list their own plant's loads");
