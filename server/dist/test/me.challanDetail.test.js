...
let challanSeq = 0;
async function createChallan(clientId, status = 'delivered') {
    challanSeq += 1;
...
beforeEach(async () => {
    await db.execute(sql `TRUNCATE TABLE challan_proof_photos, challans, orders, clients, audit_logs, users, login_attempts RESTART IDENTITY CASCADE`);
});
after(async () => {
    await pool.end();
});
test('a client reads their own challan detail with resolved proof photos', async (t) => {
    t.mock.method(proofPhotoStore, 'resolve', async (stored) => stored ? `https://signed.example/${stored.replace(/^\/objects\//, '')}` : null);
