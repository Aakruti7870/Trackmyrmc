...
function openCapturingStream(token) {
    return new Promise((resolve, reject) => {
...
test('a client connection receives events for its own company but not another', async () => {
    ...
    await waitFor(() => getSSEClientCount() === before + 1, 'the client connection to be registered with its linked-client identity');
    ...
    await waitFor(() => stream.received().includes('"tag":"own"'), "the owning company's targeted event to reach the client connection");
    assert.ok(!stream.received().includes('"tag":"foreign"'), "another company's targeted event must NOT reach this client connection");
    stream.close();
...
test('a driver connection receives events for its own trips but not another driver', async () => {
    ...
    await waitFor(() => getSSEClientCount() === before + 1, 'the driver connection to be registered with its linked-driver identity');
    ...
    await waitFor(() => stream.received().includes('"tag":"own"'), "the driver's own assigned-trip event to reach the connection");
    assert.ok(!stream.received().includes('"tag":"foreign"'), "another driver's targeted event must NOT reach this driver connection");
    stream.close();
...
test('a staff connection receives every targeted event regardless of owner', async () => {
    ...
    await waitFor(() => getSSEClientCount() === before + 1, 'the staff connection to be registered with its staff identity');
    ...
    await waitFor(() => stream.received().includes('"tag":"client-scoped"')
        && stream.received().includes('"tag":"driver-scoped"'), 'both client- and driver-scoped events to reach the staff connection');
    stream.close();
