...
async function columnExists(db, table, column) {
    const res = await db.execute(sql `SELECT 1 FROM information_schema.columns WHERE table_name = ${table} AND column_name = ${column}`);
    return res.rows.length > 0;
}
export async function migrateFleetProductionPlant(db, log = () => { }) {
    ...
    await db.execute(sql `ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS plant_id integer REFERENCES plants(id)`);
    log(hadVehicleCol ? '  – vehicles.plant_id already present' : '  ✔ added vehicles.plant_id');
    await db.execute(sql `ALTER TABLE batch_records ADD COLUMN IF NOT EXISTS plant_id integer REFERENCES plants(id)`);
