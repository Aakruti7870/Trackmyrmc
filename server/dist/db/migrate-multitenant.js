...
async function backfillPlantCodes(db, log) {
...
async function resolveHomePlant(db) {
    ...
    if (existing)
        return { id: existing.id, plantCode: existing.plantCode ?? 'PLT-001', created: false };
...
async function backfillCustomerCodes(db, log) {
...
async function backfillMappings(db) {
    ...
    for (const row of linked) {
        if (row.clientId == null || row.plantId == null)
            continue;
...
export async function migrateMultiTenant(db, log = () => { }) {
    const plantCodesAssigned = await backfillPlantCodes(db, log);
    const home = await resolveHomePlant(db);
    if (home.created)
        log(`  ✔ created home plant #${home.id} (${home.plantCode})`);
