...
export const DEFAULT_META_TEMPLATE_NAMES = {
...
export function verifyMetaWebhookSignature(signatureHeader, rawBody) {
...
export async function sendWhatsAppText(toPhone, bodyText) {
    const to = normalizePhone(toPhone);
    if (!to)
        return { ok: false, channel: 'dev', error: 'No valid recipient phone number.', retryable: false };
    const text = bodyText.trim();
    if (!text)
        return { ok: false, channel: 'dev', error: 'Message text is empty.', retryable: false };
    const meta = metaWhatsAppConfig();
    if (meta) {
        try {
            const res = await fetch(`https://graph.facebook.com/${meta.version}/${meta.phoneNumberId}/messages`, {
                method: 'POST',
                headers: {
