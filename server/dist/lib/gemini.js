...
export async function transcribeAudio(audioBase64, mimeType) {
    ...
    if (!base || !key)
        throw new GeminiError('Speech-to-text is not configured.', 503);
    ...
    const payload = {
        contents: [{
                ...
                parts: [
                    {
                        text: 'Transcribe the spoken words in this audio to plain text. Return ONLY ' +
    ...
    try {
        ...
        throw new GeminiError(`Could not reach the speech service: ${err.message}`, 502);
    }
    if (!res.ok) {
...
export async function extractDocumentFields(input) {
    ...
    if (input.type === 'image') {
        ...
        try {
            ...
            throw new GeminiError(`Could not reach the extraction service: ${err.message}`, 502);
        }
        if (!res.ok)
    ...
    try {
        ...
        return {
            ...
            confidence: ['high', 'low', 'none'].includes(parsed.confidence) ? parsed.confidence : 'low',
        };
    }
    catch {
