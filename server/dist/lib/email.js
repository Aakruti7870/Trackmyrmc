...
export async function sendPasswordResetEmail(toEmail, toName, resetUrl, expiresAt) {
    const cfg = await getSmtpConfig();
    const transporter = transporterFor(cfg);
    if (!transporter) {
        console.warn('[email] SMTP not configured (SMTP_HOST / SMTP_USER / SMTP_PASS missing). ' +
...
export async function sendLoginCodeEmail(toEmail, toName, code, expiresMinutes) {
    const cfg = await getSmtpConfig();
    const transporter = transporterFor(cfg);
    if (!transporter) {
        console.warn('[email] SMTP not configured (SMTP_HOST / SMTP_USER / SMTP_PASS missing). ' +
...
export async function sendBatchLoggedEmail(toEmails, details) {
