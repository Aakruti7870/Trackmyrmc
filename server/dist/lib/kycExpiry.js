...
const ALERT_ROLES = ['authority', 'admin', 'plant_owner'];
...
export async function tickKycExpiryAlerts() {
    ...
    const claimed = await db
        ...
        .where(and(isNull(kycDocuments.expiryAlertedAt), isNotNull(kycDocuments.expiryDate), lte(kycDocuments.expiryDate, cutoff)))
        .returning({
        id: kycDocuments.id,
    ...
    for (const doc of claimed) {
        try {
            ...
            emitSSEEvent('kyc.expiring', { profileId: profile.id, docType: doc.docType, expiryDate: doc.expiryDate, subject }, { roles: [...ALERT_ROLES], plantId: profile.plantId });
            ...
            const recipientFilter = profile.plantId == null
                ...
                : and(inArray(users.role, [...ALERT_ROLES]), eq(users.isActive, true), isNull(users.deletedAt), eq(users.plantId, profile.plantId));
            const recipients = (await db.select({ email: users.email }).from(users).where(recipientFilter))
                .map((r) => r.email)
                .filter((e) => !!e && !e.endsWith('@otp.local'));
