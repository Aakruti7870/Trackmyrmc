...
const WHATSAPP_FAILURE_ALERT_ROLES = ['admin', 'dispatcher', 'authority', 'plant_operator'];
...
// Best-effort: never throws back into the originating request.
export async function notifyOrderCancelled(orderId, reason) {
    try {
        const [row] = await db
            .select({
            plantId: orders.plantId,
        ...
        emitSSEEvent('order.cancelled', { orderId, orderNo: row.orderNo, grade: row.grade, quantity: row.quantity, clientName: row.clientName, reason: reason ?? null }, { roles: [...ORDER_APPROVAL_ROLES], plantId: row.plantId ?? undefined });
        if (row.plantId != null) {
            try {
                await sendPushToStaff(row.plantId, ORDER_APPROVAL_ROLES, {
...
export async function notifyOrderRescheduled(orderId, opts = {}) {
    try {
        const [row] = await db
            .select({
            plantId: orders.plantId,
        ...
        emitSSEEvent('order.rescheduled', { orderId, orderNo: row.orderNo, deliveryDate: opts.deliveryDate ?? null, deliveryTime: opts.deliveryTime ?? null }, { roles: [...ORDER_APPROVAL_ROLES], plantId: row.plantId ?? undefined });
        if (row.plantId != null) {
            try {
                await sendPushToStaff(row.plantId, ORDER_APPROVAL_ROLES, {
...
const BILL_REQUEST_ROLES = ['admin', 'plant_owner', 'accountant'];
...
export async function notifyBillRequested(userId, clientIds, opts = {}) {
    try {
        ...
        const plantIds = [...new Set(rows.map(r => r.plantId).filter((p) => p != null))];
        ...
        const targets = plantIds.length ? plantIds : [null];
        for (const plantId of targets) {
            emitSSEEvent('billing.request', { customerName, note: opts.note ?? null }, { roles: [...BILL_REQUEST_ROLES], plantId: plantId ?? undefined });
            try {
...
export async function notifyStaffOfWhatsAppFailure(messageSid) {
    try {
        const [row] = await db
            .select({
        ...
        try {
            const event = row.event === 'dispatch' || row.event === 'delivery' ? row.event : 'order';
            const staff = await db
                .select({ email: users.email })
                .from(users)
                .where(and(isNull(users.deletedAt), eq(users.isActive, true), inArray(users.role, [...WHATSAPP_FAILURE_ALERT_ROLES])));
            const emails = new Set();
            for (const s of staff)
                if (s.email)
                    emails.add(s.email);
