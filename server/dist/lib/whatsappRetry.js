...
export function backoffDelayMs(attemptsMade) {
...
export async function runDueWhatsAppRetries(now = new Date()) {
    const result = { sent: 0, retried: 0, gaveUp: 0 };
    for (;;) {
...
async function recordGiveUp(row, reason, lastError) {
    ...
    const why = reason === 'attempts_exhausted'
        ...
        : 'permanently';
    const detail = `WhatsApp ${event} update to ${row.toPhone} was not delivered (gave up ${why}). ` +
    ...
    try {
        emitSSEEvent('whatsapp.gave_up', { event, toPhone: row.toPhone, reason, totalAttempts, lastError }, { roles: GIVE_UP_ALERT_ROLES });
    }
    catch (err) {
...
export async function listPendingWhatsAppRetries(limit = 200) {
    const rows = await db
        .select({
...
export async function cancelWhatsAppRetry(id) {
...
export async function retryWhatsAppNow(id, now = new Date()) {
    const claimed = await db.transaction(async (tx) => {
        const [row] = await tx
            .select()
            .from(whatsappRetries)
