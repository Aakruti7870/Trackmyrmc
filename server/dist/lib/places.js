...
const VALID_CATEGORY_KEYS = new Set(SUPPLIER_CATEGORIES.map(c => c.key));
export function isSupplierCategoryKey(key) {
...
async function searchPlaces(textQuery, lat, lng, radiusKm) {
...
export async function discoverSuppliers(lat, lng, radiusKm, categoryKeys) {
    const key = placesApiKey();
    if (!key)
        throw new Error('Places API key not configured');
    ...
    const settled = await Promise.allSettled(cats.map(async (c) => ({ key: c.key, places: await searchPlaces(c.query, lat, lng, radiusKm) })));
    ...
    for (const r of settled) {
        ...
        for (const p of r.value.places) {
            ...
            if (existing) {
                if (!existing.categories.includes(r.value.key))
                    existing.categories.push(r.value.key);
            }
            else {
