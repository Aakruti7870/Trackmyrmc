...
export const DRIVER_TRIP_STATUSES = ['in_transit', 'reached_site', 'unloading'];
...
const STAGE_ORDER = ['dispatched', 'in_transit', 'reached_site', 'unloading', 'delivered'];
export function tripStageRank(status) {
...
// the dispatch flow.
export async function createTripSessionForChallan(challanId) {
    try {
        const [c] = await db
            .select({
            id: challans.id,
...
export async function getTripByChallan(challanId) {
...
export async function updateTripLocation(challanId, lat, lng) {
    try {
        await db
            .update(tripSessions)
...
export async function advanceTripStatus(challanId, status) {
...
export async function closeTripOnDelivery(challanId) {
    try {
        const [row] = await db
            .update(tripSessions)
...
export async function activeTripForDriver(driverId) {
    const rows = await db
        .select({ id: tripSessions.id, challanId: tripSessions.challanId, status: tripSessions.status })
        .from(tripSessions)
        .where(and(eq(tripSessions.driverId, driverId), inArray(tripSessions.status, ['dispatched', 'in_transit', 'reached_site', 'unloading'])))
