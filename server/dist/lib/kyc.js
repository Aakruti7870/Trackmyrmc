...
async function resolve() {
    ...
    const pick = (key, env, dflt = '') => (s[key] ?? env ?? dflt).trim();
...
export async function isKycConfigured() {
...
export async function updateKycSettings(patch) {
    if (patch.enabled !== undefined)
        await setSetting(KYC_KEYS.enabled, String(patch.enabled));
    if (patch.provider !== undefined)
        await setSetting(KYC_KEYS.provider, patch.provider.trim() || null);
    if (patch.baseUrl !== undefined)
        await setSetting(KYC_KEYS.baseUrl, patch.baseUrl.trim() || null);
    if (patch.apiVersion !== undefined)
        await setSetting(KYC_KEYS.apiVersion, patch.apiVersion.trim() || null);
    if (patch.apiKey !== undefined)
        await setSetting(KYC_KEYS.apiKey, patch.apiKey.trim() || null);
    if (patch.apiSecret !== undefined)
        await setSetting(KYC_KEYS.apiSecret, patch.apiSecret.trim() || null);
}
...
export function newProviderRef() {
...
export function toMaskedAadhaar(raw) {
    if (!raw)
        return undefined;
...
class SandboxKycProvider {
    ...
    async fetch(txnId) {
        ...
        // as one.
        const maskedAadhaar = toMaskedAadhaar(firstOf(data, ['masked_aadhaar', 'masked_aadhaar_number', 'aadhaar_number']));
...
export async function getKycProvider() {
    ...
    if (!config.configured)
        throw new KycError('KYC is not configured');
