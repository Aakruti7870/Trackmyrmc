...
import { generateOtpCode, hashOtpCode, isMetaOtpConfigured, sendCodeViaMetaWhatsApp, } from './otp.js';
...
export async function resolveStaffForOtp(email) {
    ...
    if (!u || u.deletedAt || !u.isActive)
        return null;
    if (!STAFF_OTP_ROLES.has(u.role))
        return null;
    return u;
}
async function clearStaffCode(userId) {
...
export async function sendStaffLoginCode(user) {
    ...
    if (channel === 'email') {
        ...
        if (canWhatsApp) {
            const w = await sendCodeViaMetaWhatsApp(user.phone, code);
            if (w.ok) {
...
export function isReviewDemoLogin(email, code) {
    const cfg = reviewDemoConfig();
    if (!cfg || email.trim().toLowerCase() !== cfg.email)
        return false;
...
export async function verifyStaffLoginCode(userId, code) {
