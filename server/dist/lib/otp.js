...
import { metaWhatsAppConfig } from './whatsapp.js';
...
export function isMetaOtpConfigured() {
...
export function metaOtpTemplateName() {
...
export function normalizePhone(input) {
    ...
    if (digits.length === 10)
        return `+91${digits}`; // bare Indian mobile
...
async function sendViaMetaOtp(cfg, phone, code) {
    try {
        const payload = {
...
export async function sendCodeViaMetaWhatsApp(phone, code) {
    const meta = metaWhatsAppConfig();
    if (!meta)
        return { ok: false, channel: 'whatsapp', error: 'WhatsApp sender is not configured.' };
