...
function confidenceFor(sampleDays) {
    if (sampleDays >= 8)
        return 'high';
    if (sampleDays >= 3)
        return 'medium';
...
export function computeForecast(args) {
    ...
    const bookedByGrade = new Map();
    for (const b of booked)
        bookedByGrade.set(b.grade, (bookedByGrade.get(b.grade) ?? 0) + b.qty);
    const recurringByGrade = new Map();
    for (const r of recurring)
        recurringByGrade.set(r.grade, (recurringByGrade.get(r.grade) ?? 0) + r.qty);
    // Per-grade history grouped by date so we can build same-weekday series and an
    // overall daily-average series.
    const grades = new Set();
    const histByGrade = new Map();
    ...
    for (const g of bookedByGrade.keys())
        grades.add(g);
    for (const g of recurringByGrade.keys())
        grades.add(g);
