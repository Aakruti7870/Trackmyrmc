...
function resultPage(kind, message) {
...
function sendPage(res, status, kind, message) {
...
router.get('/callback', async (req, res) => {
    ...
    if (row.status !== 'pending') {
        ...
        sendPage(res, 200, row.status === 'verified' ? 'success' : 'failed', row.status === 'verified' ? 'Your identity is verified.' : 'Verification was not successful.');
        return;
    }
    ...
    const [claimed] = await db.update(kycVerifications).set({ callbackClaimedAt: new Date() })
        .where(and(eq(kycVerifications.id, row.id), eq(kycVerifications.status, 'pending'), isNull(kycVerifications.callbackClaimedAt), gt(kycVerifications.expiresAt, new Date()))).returning({ id: kycVerifications.id });
    if (!claimed) {
...
router.get('/status', requireRole('client'), async (req, res) => {
    ...
    const [expired] = await db
        ...
        .where(and(eq(kycVerifications.userId, actor.id), eq(kycVerifications.status, 'pending'), sql `${kycVerifications.expiresAt} < now()`))
    ...
    const nameUpdateFailed = rawStatus === 'verified' &&
        !!latest?.fullName &&
