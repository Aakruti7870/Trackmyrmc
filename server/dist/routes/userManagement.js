 ...
 */
function resolvePlantParam(req, res) {
...
function plantStatus(p) {
    if (!p.isActive)
        return 'suspended';
    if (p.subscriptionStatus === 'suspended' || p.subscriptionStatus === 'cancelled')
        return 'suspended';
    return 'active';
}
// ---------------------------------------------------------------------------
...
router.get('/plants', async (req, res) => {
    ...
    if (scope === null && !isPlatformStaff(actor)) {
        res.json([]);
        return;
    }
    ...
    const counts = await db.select({
        ...
        staff: sql `count(*) filter (where ${users.role} not in ('client'))::int`,
        customers: sql `count(*) filter (where ${users.role} = 'client')::int`,
    }).from(users)
    ...
    for (const c of counts) {
        if (c.plantId !== null)
            countMap.set(c.plantId, { staff: c.staff, customers: c.customers });
    }
    res.json(rows.map(p => ({
        id: p.id,
        name: p.name,
...
router.get('/plants/:plantId/users', async (req, res) => {
    ...
    const rows = await db.select({
        ...
        .where(and(eq(users.plantId, plantId), deletedOnly ? isNotNull(users.deletedAt) : isNull(users.deletedAt)))
...
function canManageRole(actor, targetRole) {
    if (targetRole === 'authority')
        return false; // never through this console
    if (targetRole === 'client')
        return true; // customers are not in the staff hierarchy
 ...
 */
async function resolveTarget(req, res, opts = {}) {
...
router.put('/users/:id', async (req, res) => {
    ...
    if (rest.role && rest.role !== target.role && rest.role !== 'client') {
        ...
        if (Number.isFinite(limit)) {
            const [{ count: held }] = await db.select({ count: sql `count(*)::int` })
                .from(users)
                .where(and(eq(users.plantId, target.plantId), eq(users.role, rest.role), isNull(users.deletedAt), ne(users.id, target.id)));
            if (held >= limit) {
    ...
    const changes = [];
    if (rest.name && rest.name !== target.name)
        changes.push('name');
    if (rest.phone !== undefined)
        changes.push('phone');
    if (rest.role && rest.role !== target.role)
        changes.push(`role ${target.role}→${rest.role}`);
    if (rest.isActive === false && target.isActive)
        changes.push('suspended');
    if (rest.isActive === true && !target.isActive)
...
router.post('/users/:id/restore', async (req, res) => {
    ...
    if (!target)
        return;
    if (!target.deletedAt) {
        res.status(400).json({ error: 'This account is not deleted.' });
        return;
    }
    const actor = req.user;
    ...
    if (target.role !== 'client') {
        ...
        if (Number.isFinite(limit)) {
            const [{ count: held }] = await db.select({ count: sql `count(*)::int` })
                .from(users)
                .where(and(eq(users.plantId, target.plantId), eq(users.role, target.role), isNull(users.deletedAt)));
            if (held >= limit) {
