...
import { getStaffPasswordLoginRoles, serializeStaffPasswordLoginRoles, STAFF_PASSWORD_LOGIN_KEY, STAFF_PASSWORD_LOGIN_ROLES, } from '../lib/staffPasswordLogin.js';
...
import { listPendingWhatsAppRetries, cancelWhatsAppRetry, retryWhatsAppNow, } from '../lib/whatsappRetry.js';
const router = Router();
...
const commissionSchema = z.object({
    pct: z
        ...
        .trim()
        .optional()
        .refine(v => v === undefined || v === '' || (Number.isFinite(Number(v)) && Number(v) >= 0 && Number(v) <= 100), 'Commission must be between 0 and 100'),
});
...
router.get('/role-permissions', async (req, res) => {
    if (!isPlatformStaff(req.user)) {
        res.status(403).json({ error: 'Only platform staff can manage role permissions.' });
        return;
    }
...
router.get('/app-version', async (req, res) => {
    if (!isPlatformStaff(req.user)) {
        res.status(403).json({ error: 'Only platform staff can manage the app version.' });
        return;
    }
...
router.post('/kyc-settings', async (req, res) => {
    ...
    const changed = [];
    if (before.enabled !== after.enabled)
        changed.push(`KYC ${after.enabled ? 'enabled' : 'disabled'}`);
    if (before.provider !== after.provider)
        changed.push(`provider set to ${after.provider}`);
    if (before.baseUrl !== after.baseUrl)
        changed.push('base URL updated');
    if (before.apiVersion !== after.apiVersion)
        changed.push('API version updated');
    if (before.hasApiKey !== after.hasApiKey)
        changed.push(after.hasApiKey ? 'API key set' : 'API key cleared');
    if (before.hasApiSecret !== after.hasApiSecret)
        changed.push(after.hasApiSecret ? 'API secret set' : 'API secret cleared');
    const detail = changed.length
...
router.post('/ai-settings', async (req, res) => {
    ...
    const changed = [];
    if (before.enabled !== after.enabled)
        changed.push(`enabled ${before.enabled} → ${after.enabled}`);
    if (before.persona !== after.persona)
        changed.push('persona updated');
    if (before.greeting !== after.greeting)
        changed.push('greeting updated');
    const actor = req.user;
    try {
        await db.insert(auditLogs).values({
            actorId: actor.id,
