...
import { assertKycTransition, canTransitionKyc, isKycState, transitionRequiresReason } from '../lib/kycStateMachine.js';
// ============================================================================
...
const REVIEW_ROLES = ['authority', 'admin', 'plant_owner'];
...
const VEHICLE_MANAGER_ROLES = ['authority', 'admin', 'plant_owner', 'supervisor', 'fleet_manager'];
const MAX_OBJECT_PATH_CHARS = 1024;
const MAX_TEXT_CHARS = 200;
const DOC_TYPES = ['gst', 'pan', 'aadhaar', 'driving_license', 'rc', 'insurance', 'puc', 'fitness', 'photo', 'other'];
// Which entity type a user-role account's own KYC profile is.
function entityTypeForRole(role) {
    if (role === 'client')
        return 'customer';
    if (role === 'driver')
        return 'driver';
...
function parsePan(value) {
    ...
    if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(p))
        throw new Error('PAN must be a valid 10-character PAN');
    return p;
}
function parseAadhaarMasked(value) {
    ...
    if (digits.length !== 4)
        throw new Error('Provide only the LAST 4 digits of Aadhaar (never the full number)');
...
function parseDocType(value) {
    if (typeof value !== 'string' || !DOC_TYPES.includes(value))
        throw new Error('Invalid document type');
    return value;
}
function parseObjectPath(value) {
    if (typeof value !== 'string' || !isObjectStoragePath(value))
        throw new Error('Document must be an uploaded object path');
    if (value.length > MAX_OBJECT_PATH_CHARS)
        throw new Error('Document path is too long');
    return value;
}
...
async function writeAudit(actor, action, profileId, targetUserId, detail) {
...
async function docsForProfile(profileId) {
    return db
        .select({
        id: kycDocuments.id,
...
async function notifySubject(profile) {
    if (!profile.userId)
        return;
    const [u] = await db
...
function requireFleetScope(actor) {
...
router.get('/me', async (req, res) => {
    ...
    if (!profile) {
        res.json({ profile: null, documents: [] });
        return;
    }
...
router.post('/me/submit', async (req, res) => {
    ...
    if (!existing) {
        res.status(404).json({ error: 'Fill in your KYC details before submitting' });
        return;
    }
    ...
    if (existing.status === 'submitted') {
        res.status(409).json({ error: 'Already submitted and awaiting review' });
        return;
    }
    ...
    if (!canTransitionKyc(existing.status, 'submitted', 'subject')) {
...
router.post('/me/documents', async (req, res) => {
    const actor = req.user;
    let docType, objectPath, expiryDate, fileName;
    try {
...
async function vehicleInScope(actor, vehicleId) {
...
router.get('/vehicles/:vehicleId', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    const actor = req.user;
    if (!requireFleetScope(actor)) {
        res.status(403).json({ error: 'Not bound to a plant' });
        return;
    }
    ...
    if (!profile) {
        res.json({ vehicle: v, profile: null, documents: [] });
        return;
    }
...
router.put('/vehicles/:vehicleId', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    ...
    if (existing) {
        if (existing.status === 'verified') {
            res.status(409).json({ error: 'A verified vehicle KYC can no longer be edited' });
            return;
        }
        const [row] = await db
            .update(kycProfiles)
...
router.post('/vehicles/:vehicleId/submit', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    ...
    const [existing] = await db.select({ id: kycProfiles.id, status: kycProfiles.status }).from(kycProfiles).where(eq(kycProfiles.vehicleId, v.id));
    if (!existing) {
        res.status(404).json({ error: 'Fill in the vehicle KYC details before submitting' });
        return;
    }
    if (existing.status === 'verified') {
        res.status(409).json({ error: 'Already verified' });
...
router.post('/vehicles/:vehicleId/documents', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    ...
    if (profile && profile.status === 'verified') {
        res.status(409).json({ error: 'A verified vehicle KYC can no longer be changed' });
        return;
    }
    if (!(await proofPhotoStore.verifyExists(objectPath))) {
        res.status(400).json({ error: 'Document upload was not found in storage' });
...
router.get('/profiles', requireRole(...REVIEW_ROLES), async (req, res) => {
    ...
    const rows = await db
        ...
        docCount: sql `(select count(*)::int from ${kycDocuments} where ${kycDocuments.profileId} = ${kycProfiles.id})`,
    })
        .from(kycProfiles)
...
router.post('/profiles/:id/:decision(approve|reject)', requireRole(...REVIEW_ROLES), async (req, res) => {
    ...
    if (existing.userId === actor.id) {
        res.status(403).json({ error: 'You cannot review your own KYC profile' });
        return;
    }
    ...
    await writeAudit(actor, decision === 'verified' ? 'kyc.verified' : 'kyc.rejected', id, existing.userId, decision === 'verified' ? 'verified the KYC profile' : `rejected the KYC profile: ${reason}`);
...
router.post('/profiles/:id/transition', requireRole(...REVIEW_ROLES), async (req, res) => {
    ...
    if (!Number.isInteger(id) || id <= 0 || !isKycState(to)) {
        res.status(400).json({ error: 'A valid profile id and KYC status are required' });
        return;
    }
    const actor = req.user;
    const scope = plantScope(actor.plantId, kycProfiles.plantId);
    ...
    if (existing.userId === actor.id) {
        res.status(403).json({ error: 'You cannot transition your own KYC profile' });
        return;
    }
    try {
        assertKycTransition(existing.status, to, 'reviewer');
    }
    catch (error) {
        res.status(409).json({ error: error instanceof Error ? error.message : 'Invalid KYC transition' });
        return;
    }
    if (transitionRequiresReason(existing.status, to) && !reason) {
        res.status(400).json({ error: 'A reason is required for this KYC transition' });
        return;
    }
    ...
    const [row] = await db.update(kycProfiles).set({
        ...
        updatedAt: new Date(),
    }).where(and(eq(kycProfiles.id, id), eq(kycProfiles.status, existing.status))).returning();
    if (!row) {
...
router.get('/badges', requireRole(...REVIEW_ROLES, 'supervisor', 'fleet_manager', 'dispatcher'), async (req, res) => {
    ...
    for (const r of rows) {
        ...
        if (key != null)
            map[key] = r.status === 'verified' ? 'approved' : r.status;
    }
    res.json(map);
});
