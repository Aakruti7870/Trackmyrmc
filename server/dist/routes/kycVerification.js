...
router.put('/vehicles/:vehicleId', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    ...
    if (existing) {
        if (existing.status === 'approved') {
            res.status(409).json({ error: 'An approved vehicle KYC can no longer be edited' });
            return;
        }
        const [row] = await db
            .update(kycProfiles)
...
router.post('/vehicles/:vehicleId/documents', requireRole(...VEHICLE_MANAGER_ROLES), async (req, res) => {
    ...
    if (profile && profile.status === 'approved') {
        res.status(409).json({ error: 'An approved vehicle KYC can no longer be changed' });
        return;
    }
    if (!(await proofPhotoStore.verifyExists(objectPath))) {
        res.status(400).json({ error: 'Document upload was not found in storage' });
...
router.post('/profiles/:id/:decision(approve|reject)', requireRole(...REVIEW_ROLES), async (req, res) => {
    ...
    const [row] = await db
        ...
        .returning();
    await writeAudit(actor, decision === 'approved' ? 'kyc.approved' : 'kyc.rejected', id, existing.userId, decision === 'approved' ? 'approved the KYC profile' : `rejected the KYC profile: ${reason}`);
