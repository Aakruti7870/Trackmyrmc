...
const STAFF_ROLES = ['admin', 'dispatcher', 'authority', 'plant_operator'];
...
async function fuelLogInScope(id, actorPlantId) {
...
function validateBillPhoto(value) {
    if (value === undefined)
        return undefined;
    if (value === null || value === '')
        return null;
    if (typeof value !== 'string')
        throw new Error('Bill photo must be a string');
    if (!isObjectStoragePath(value))
        throw new Error('Bill photo must be an uploaded object path');
    if (value.length > MAX_OBJECT_PATH_CHARS)
        throw new Error('Bill photo path is too long');
    return value;
}
...
function parseOptionalOdometer(value) {
    if (value === undefined || value === null || value === '')
        return null;
    const n = Number(value);
...
function parseFilledAt(value) {
    ...
    if (Number.isNaN(d.getTime()))
        throw new Error('Filled-at must be a valid date');
    return d;
}
...
router.get('/', async (req, res) => {
    ...
    if (scope)
        filters.push(scope);
    if (vehicleId)
        filters.push(eq(fuelLogs.vehicleId, +vehicleId));
    if (from)
        filters.push(gte(fuelLogs.filledAt, new Date(from)));
    if (to)
        filters.push(lte(fuelLogs.filledAt, new Date(to)));
...
router.post('/', async (req, res) => {
    ...
    let litres, amount, odometer, filledAt, billPhoto;
    try {
...
router.put('/:id', async (req, res) => {
    ...
    if (!(await fuelLogInScope(id, req.user.plantId))) {
        res.status(404).json({ error: 'Not found' });
        return;
    }
    const updateData = {};
    try {
        ...
        if (req.body.litres !== undefined)
            updateData.litres = parseLitres(req.body.litres);
        if (req.body.amount !== undefined)
            updateData.amount = parseOptionalAmount(req.body.amount);
        if (req.body.odometer !== undefined)
            updateData.odometer = parseOptionalOdometer(req.body.odometer);
        if (req.body.filledAt !== undefined)
            updateData.filledAt = parseFilledAt(req.body.filledAt);
