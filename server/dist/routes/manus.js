...
import { isManusConfigured, ManusError, createTask, listTasks, getTaskDetail, listTaskMessages, sendTaskMessage, } from '../lib/manus.js';
...
function sendManusError(res, err) {
...
function readPrompt(body) {
    ...
    if (!trimmed || trimmed.length > MAX_PROMPT_LEN)
        return null;
    return trimmed;
}
...
router.post('/tasks', rateLimit({ windowMs: 60_000, max: 20, name: 'manus-create' }), async (req, res) => {
...
router.post('/tasks/:id/messages', rateLimit({ windowMs: 60_000, max: 30, name: 'manus-message' }), async (req, res) => {
    const prompt = readPrompt(req.body);
    if (!prompt) {
