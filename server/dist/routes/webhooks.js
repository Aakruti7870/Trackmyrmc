...
router.post('/whatsapp', async (req, res) => {
    ...
    if (verification === 'unconfigured' && isProd()) {
        console.error('[whatsapp] Rejecting Meta webhook POST: WHATSAPP_META_APP_SECRET is not set, so the X-Hub-Signature-256 cannot be verified.');
        res.sendStatus(403);
        return;
    }
    try {
...
async function applyStatus(st) {
    ...
    if (!sid || !status)
        return;
    const errorCode = Array.isArray(st?.errors) && st.errors[0]?.code != null ? String(st.errors[0].code) : null;
